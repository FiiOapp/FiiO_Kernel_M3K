...
static unsigned int audio_rate [] = {
        192000,176400,96000,88200,64000,48000,44100,32000,
        24000,22050,16000,12000,11025,8000,6000,
};
	...
	for (tmp1=1; tmp1<max1; tmp1++)
		for(tmp2=1; tmp2<max2; tmp2++)
			if(tmp1*tmp2 == machval)
				break;
	if(tmp1*tmp2 != machval){
		printk("can't find mach val:max1=%d,max2=%d,machval=%d\n",max1,max2,machval);
		return -1;
	}
	*res1 = tmp1;
	*res2 = tmp2;
...
/*
int rate_to_number(unsigned long rate)
{
    switch(rate)
    {
        case 192000:
...
static int cgu_audio_calculate_set_rate(struct clk* clk, unsigned long rate, unsigned int pid){
	...
	m = audio_div[i].savem;
    printk("audio_div[%d].savem = %d,audio_div[%d].saven = %d\n",i,audio_div[i].savem,i,audio_div[i].saven);
	printk("audiodiv_apll[i].rate = %d, audiodiv_apll[i].savem =  %d, audiodiv_apll[i].saven = %d\n",
		   audiodiv_apll[i].rate, audiodiv_apll[i].savem, audiodiv_apll[i].saven);
    printk("audiodiv_mpll[i].rate = %d, audiodiv_mpll[i].savem =  %d, audiodiv_mpll[i].saven = %d\n",
           audiodiv_mpll[i].rate, audiodiv_mpll[i].savem, audiodiv_mpll[i].saven);
	if(no == CGU_AUDIO_I2S) {//no == CGU_AUDIO_I2S
        printk("no == CGU_AUDIO_I2S\n");
#ifdef CONFIG_SND_ASOC_JZ_AIC_SPDIF_V13
		m *= 2;
#endif
		d_max = 0x1ff;
		tmp_val = audio_div[i].saven / 64;
        printk("n_max:%d,d_max:%d,tmp_val:%d",n_max,d_max,tmp_val);
		if(tmp_val > n_max){
		...
		if(tmp_val&cgu_clks[no].en){
            printk("enter %s,line:%d,tmp_val=%d,cgu_clks[no].off=%d,1111\n",__func__,__LINE__,tmp_val,cgu_clks[no].off);
			cpm_outl(tmp_val,cgu_clks[no].off);
		}else{
            printk("enter %s,line:%d,2222\n",__func__,__LINE__);
			cgu_clks[no].cache = tmp_val;
		}

        printk("enter %s,line:%d,rate=%lu,d=%d\n",__func__,__LINE__,rate,d);
		writel(d-1,(volatile unsigned int*)I2S_PRI_DIV);//rate_to_number(rate)

		spin_unlock_irqrestore(&cpm_cgu_lock,flags);
	}else if (no == CGU_AUDIO_PCM) {//no == CGU_AUDIO_PCM
        printk("no == CGU_AUDIO_PCM\n");
