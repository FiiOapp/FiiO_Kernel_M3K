...
static int cgu_enable(struct clk *clk,int on)
{
	...
	if(no == CGU_AIC){
		...
		/* Should unmask aic clock gate before enable cgu_aic */
		if ((*(volatile unsigned int*)0xb0000060) & (1 << 31)) {
			...
			printk("cgu_enable() i2s clock is EXT_CLK/2 ...\n");
			return 0;
		}
	}
	...
	if(no == CGU_AIC){
		/* Why restore?  -- lingang.wang@ingenic.com
		REG32(AICFR) = aictmp;
		REG32(AICFR) = i2stmp; // should be I2SCR?
		*/
	}
