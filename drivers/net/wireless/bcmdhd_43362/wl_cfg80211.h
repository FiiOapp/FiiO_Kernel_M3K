...
static inline u32 wl_get_status_all(struct wl_priv *wl, s32 status)
{
	struct net_info *_net_info, *next;
	u32 cnt = 0;
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
...
static inline void wl_set_status_all(struct wl_priv *wl, s32 status, u32 op)
{
	struct net_info *_net_info, *next;
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
		switch (op) {
		...
		case 2:
			...
			if (wl->state_notifier &&
				...
				wl->state_notifier(wl, _net_info, status,
						   false);
			break;
		case 4:
	...
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
		if (ndev && (_net_info->ndev == ndev)) {
			...
			case 1:
				...
				if (wl->state_notifier &&
				    test_bit(status, &(wl->interrested_state)))
					wl->state_notifier(wl, _net_info,
							   status, true);
				break;
			case 2:
				clear_bit(status, &_net_info->sme_state);
...
static inline struct wl_profile *wl_get_profile_by_netdev(struct wl_priv *wl,
							  struct net_device
							  *ndev)
{
	struct net_info *_net_info, *next;
...
static inline struct net_info *wl_get_netinfo_by_netdev(struct wl_priv *wl,
							struct net_device *ndev)
{
	struct net_info *_net_info, *next;
