...
wl_cfgp2p_vndr_ie(struct wl_priv *wl, u8 * iebuf, s32 pktflag,
		  s8 * oui, s32 ie_id, s8 * data, s32 datalen,
...
bool wl_cfgp2p_is_gas_action(void *frame, u32 frame_len)
{
	...
	if (sd_act_frm->action == P2PSD_ACTION_ID_GAS_IRESP)
		return wl_cfgp2p_find_gas_subtype(P2PSD_GAS_OUI_SUBTYPE,
						  (u8 *) sd_act_frm->
						  query_data + GAS_RESP_OFFSET,
						  frame_len);

	else if (sd_act_frm->action == P2PSD_ACTION_ID_GAS_CRESP)
		return wl_cfgp2p_find_gas_subtype(P2PSD_GAS_OUI_SUBTYPE,
						  (u8 *) sd_act_frm->
						  query_data + GAS_CRESP_OFFSET,
						  frame_len);
...
void wl_cfgp2p_print_actframe(bool tx, void *frame, u32 frame_len, u32 channel)
{
	...
	if (wl_cfgp2p_is_pub_action(frame, frame_len)) {
		...
		case P2P_PAF_GON_REQ:
			CFGP2P_ACTION(("%s P2P Group Owner Negotiation Req Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		case P2P_PAF_GON_RSP:
			CFGP2P_ACTION(("%s P2P Group Owner Negotiation Rsp Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		case P2P_PAF_GON_CONF:
			CFGP2P_ACTION(("%s P2P Group Owner Negotiation Confirm Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		...
		case P2P_PAF_DEVDIS_REQ:
			CFGP2P_ACTION(("%s P2P Device Discoverability Request Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		case P2P_PAF_DEVDIS_RSP:
			CFGP2P_ACTION(("%s P2P Device Discoverability Response Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		case P2P_PAF_PROVDIS_REQ:
			CFGP2P_ACTION(("%s P2P Provision Discovery Request Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		case P2P_PAF_PROVDIS_RSP:
			CFGP2P_ACTION(("%s P2P Provision Discovery Response Frame," " channel=%d\n", (tx) ? "TX" : "RX", channel));
			break;
		default:
 ...
 * Returns 0 if success.
 */
s32 wl_cfgp2p_ifdisable(struct wl_priv * wl, struct ether_addr * mac)
{
	s32 ret;
...
#define P2PAPI_SCAN_AF_SEARCH_DWELL_TIME_MS 100
	...
	if (search_state == WL_P2P_DISC_ST_SEARCH) {
		...
		memset(&ssid.SSID, 0, sizeof(ssid.SSID));
		memcpy(&ssid.SSID, WL_P2P_WILDCARD_SSID,
...
static s32
wl_cfgp2p_parse_vndr_ies(u8 * parse, u32 len, struct parsed_vndr_ies *vndr_ies)
{
	s32 err = BCME_OK;
	vndr_ie_t *vndrie;
	bcm_tlv_t *ie;
...
#define IE_TYPE_LEN(type, bsstype) (wl_to_p2p_bss_saved_ie(wl, bsstype).p2p_ ## type ## _ie_len)
	...
	if (vndr_ie_len > mgmt_ie_buf_len) {
		...
		if (mgmt_ie_buf != NULL) {
			if (parsed_ie_buf_len
			    && (parsed_ie_buf_len == *mgmt_ie_len)
			    &&
			    (memcmp(mgmt_ie_buf, curr_ie_buf, parsed_ie_buf_len)
			     == 0)) {
			...
			for (i = 0; i < old_vndr_ies.count; i++) {
				...
				CFGP2P_INFO(("DELETED ID : %d, Len: %d , OUI:%02x:%02x:%02x\n", vndrie_info->vndrie.id, vndrie_info->vndrie.len, vndrie_info->vndrie.oui[0], vndrie_info->vndrie.oui[1], vndrie_info->vndrie.oui[2]));

				del_add_ie_buf_len =
				    wl_cfgp2p_vndr_ie(wl, curr_ie_buf, pktflag,
						      vndrie_info->vndrie.oui,
						      vndrie_info->vndrie.id,
						      vndrie_info->ie_ptr +
						      VNDR_IE_FIXED_LEN,
						      vndrie_info->ie_len -
						      VNDR_IE_FIXED_LEN, "del");
		...
		*mgmt_ie_len = 0;
		...
		if (mgmt_ie_buf && parsed_ie_buf_len) {
			...
			for (i = 0; i < new_vndr_ies.count; i++) {
				...
				CFGP2P_INFO(("ADDED ID : %d, Len: %d(%d), OUI:%02x:%02x:%02x\n", vndrie_info->vndrie.id, vndrie_info->vndrie.len, vndrie_info->ie_len - 2, vndrie_info->vndrie.oui[0], vndrie_info->vndrie.oui[1], vndrie_info->vndrie.oui[2]));
				...
				del_add_ie_buf_len =
				    wl_cfgp2p_vndr_ie(wl, curr_ie_buf, pktflag,
						      ...
						      VNDR_IE_FIXED_LEN,
						      vndrie_info->ie_len -
						      VNDR_IE_FIXED_LEN, "add");

				/* verify remained buf size before copy data */
				if (remained_buf_len >= vndrie_info->ie_len) {
					remained_buf_len -= vndrie_info->ie_len;
				} else {
					CFGP2P_ERR(("no space in mgmt_ie_buf: pktflag = %d, " "found vndr ies # = %d(cur %d), remained len %d, " "cur mgmt_ie_len %d, new ie len = %d\n", pktflag, new_vndr_ies.count, i, remained_buf_len, *mgmt_ie_len, vndrie_info->ie_len));
					break;
				}

				/* save the parsed IE in wl struct */
				memcpy(ptr + (*mgmt_ie_len),
				       vndrie_info->ie_ptr,
				       vndrie_info->ie_len);
		...
		if (total_ie_buf_len) {
			ret =
			    wldev_iovar_setbuf_bsscfg(ndev, "vndr_ie",
						      g_mgmt_ie_buf,
						      total_ie_buf_len,
						      wl->ioctl_buf,
						      WLC_IOCTL_MAXLEN, bssidx,
...
wifi_wfd_ie_t *wl_cfgp2p_find_wfdie(u8 * parse, u32 len)
{
	bcm_tlv_t *ie;
...
struct net_device *wl_cfgp2p_find_ndev(struct wl_priv *wl, s32 bssidx)
{
	u32 i;
...
s32 wl_cfgp2p_find_type(struct wl_priv * wl, s32 bssidx, s32 * type)
{
	u32 i;
			  ...
			  const wl_event_msg_t * e, void *data)
{
	...
	if (wl_get_p2p_status(wl, LISTEN_EXPIRED) == 0) {
		...
		if (wl->afx_hdl->is_listen == TRUE &&
			...
			wl_clr_drv_status(wl, WAITING_NEXT_ACT_FRM_LISTEN,
					  ndev);
			WL_DBG(("Listen DONE and wake up wait_next_af !!(%d)\n",
				jiffies_to_msecs(jiffies -
						 wl->af_tx_sent_jiffies)));
...
void wl_cfgp2p_listen_expired(unsigned long data)
{
	...
	wl_cfg80211_event(wl->p2p_net ? wl->p2p_net :
			  wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_DEVICE), &msg,
			  NULL);
#else
			     ...
			     const wl_event_msg_t * e, void *data)
{
	...
	if (wl_get_drv_status_all(wl, SENDING_ACT_FRM)) {
		if (event_type == WLC_E_ACTION_FRAME_COMPLETE) {
			...
			CFGP2P_INFO((" WLC_E_ACTION_FRAME_OFFCHAN_COMPLETE is received," "status : %d\n", status));
...
exit:
	...
	wl_cfg80211_add_to_eventbuffer(&buf,
				       WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE,
				       false);
	wl_cfg80211_add_to_eventbuffer(&buf, WLC_E_ACTION_FRAME_COMPLETE,
				       false);
	if ((evt_ret =
	     wl_cfg80211_apply_eventbuffer(wl_to_prmry_ndev(wl), wl,
					   &buf)) < 0) {
...
u8 *wl_cfgp2p_find_attrib_in_all_p2p_Ies(u8 * parse, u32 len, u32 attrib)
{
	bcm_tlv_t *ie;
	u8 *pAttrib;

	CFGP2P_INFO(("Starting parsing parse %p attrib %d remaining len %d ",
		     parse, attrib, len));
	while ((ie = bcm_parse_tlvs(parse, (int)len, DOT11_MNG_VS_ID))) {
		if (wl_cfgp2p_is_p2p_ie((uint8 *) ie, &parse, &len) == TRUE) {
			/* Have the P2p ie. Now check for attribute */
			if ((pAttrib =
			     wl_cfgp2p_retreive_p2pattrib(parse,
							  attrib)) != NULL) {
				CFGP2P_INFO(("P2P attribute %d was found at parse %p", attrib, parse));
				return pAttrib;
			} else {
				parse += (ie->len + TLV_HDR_LEN);
				len -= (ie->len + TLV_HDR_LEN);
				CFGP2P_INFO(("P2P Attribute %d not found Moving parse" " to %p len to %d", attrib, parse, len));
			}
		} else {
			/* It was not p2p IE. parse will get updated automatically to next TLV */
			CFGP2P_INFO(("IT was NOT P2P IE parse %p len %d", parse,
				     len));
		}
	}
...
u8 *wl_cfgp2p_retreive_p2p_dev_addr(wl_bss_info_t * bi, u32 bi_length)
{
	...
	if ((capability =
	     wl_cfgp2p_find_attrib_in_all_p2p_Ies(((u8 *) bi) + bi->ie_offset,
						  bi->ie_length,
						  P2P_SEID_P2P_INFO)) == NULL) {
	...
	if (!
		 ...
		 */
		ptr =
		    wl_cfgp2p_find_attrib_in_all_p2p_Ies(((u8 *) bi) +
							 bi->ie_offset,
							 bi->ie_length,
							 P2P_SEID_DEV_ID);
	}
...
static void
wl_cfgp2p_ethtool_get_drvinfo(struct net_device *net,
...
#if defined(WL_CFG80211_P2P_DEV_IF)
struct wireless_dev *wl_cfgp2p_add_p2p_disc_if(void)
{
	extern struct wl_priv *wlcfg_drv_priv;
