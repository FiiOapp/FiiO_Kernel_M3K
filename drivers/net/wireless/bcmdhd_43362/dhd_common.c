...
void dhd_common_init(osl_t * osh)
{
	...
	bcm_strncpy_s(conf_path, sizeof(conf_path), CONFIG_BCMDHD_CONFIG_PATH,
		      MOD_PARAM_PATHLEN - 1);
	...
	case IOV_GVAL(IOV_HOSTREORDER_FLOWS):
		{
			...
			for (i = 0; i < WLHOST_REORDERDATA_MAXFLOWS; i++) {
				if (dhd_pub->reorder_bufs[i] != NULL) {
					*ptr =
					    dhd_pub->reorder_bufs[i]->flow_id;
					ptr++;
					count++;
				}
			}
 ...
 */
bool dhd_prec_drop_pkts(osl_t * osh, struct pktq * pq, int prec)
{
	struct pktq_prec *q = NULL;
	void *p, *prev = NULL, *next = NULL, *first = NULL, *last =
	    NULL, *prev_first = NULL;
	...
	if ((p == NULL)
	    || ((frag_info != DHD_PKT_FRAG_NONE) && !(first && last))) {
			...
			if (hdr.trace_type == MSGTRACE_HDR_TYPE_MSG) {
				...
				if (ntoh32(hdr.seqnum) == logtrace_seqnum_prev) {
					...
					printf("Logtrace %x timestamp %x %x",
					       logtrace_seqnum_prev, timestamp,
					       w);
...
void dhd_pktfilter_offload_delete(dhd_pub_t * dhd, int id)
{
	...
	bcm_mkiovar("pkt_filter_delete", (char *)&id, 4, iovbuf,
		    sizeof(iovbuf));
	ret =
...
void dhd_arp_offload_enable(dhd_pub_t * dhd, int arp_enable)
{
	...
	if (arp_enable) {
		...
		retcode =
		    dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbuf, sizeof(iovbuf),
				     FALSE, 0);
		if (retcode) {
...
void dhd_aoe_arp_clr(dhd_pub_t * dhd, int idx)
{
	...
	if ((ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE,
			      idx)) < 0)
...
void dhd_arp_offload_add_ip(dhd_pub_t * dhd, uint32 ipaddr, int idx)
{
	...
	retcode =
	    dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, idx);
...
int dhd_get_suspend_bcn_li_dtim(dhd_pub_t * dhd)
{
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_BCNPRD,
				    &ap_beacon, sizeof(ap_beacon), FALSE,
				    0)) < 0) {
	...
	DHD_ERROR(("%s beacon=%d bcn_li_dtim=%d DTIM=%d Listen=%d\n",
		   __FUNCTION__, ap_beacon, bcn_li_dtim, dtim_assoc,
