		   ...
		   uint8 * byte)
{
	...
	if (rw) {		/* CMD52 Write */
			...
			/* to allow abort command through F1 */
				...
				while (sdio_abort_retry--) {
					if (gInstance->func[func]) {
						 ...
						 */
						sdio_writeb(gInstance->
							    func[func], *byte,
							    regaddr, &err_ret);
						sdio_release_host(gInstance->
								  func[func]);
					}
					if (!err_ret)
						break;
		...
		if (gInstance->func[func]) {
			...
			if (func == 0) {
				*byte =
				    sdio_f0_readb(gInstance->func[func],
						  regaddr, &err_ret);
			} else {
				*byte =
		   ...
		   uint addr, uint32 * word, uint nbytes)
{
	...
	return (((err_ret == 0)
		 && (err_ret2 ==
		     0)) ? SDIOH_API_RC_SUCCESS : SDIOH_API_RC_FAIL);
}
...
bool sdioh_glom_enabled(void)
{
	return sd_txglom;
}
#endif /* BCMSDIOH_TXGLOM */

static INLINE int sdioh_request_packet_align(uint pkt_len, uint write,
					     uint func, int blk_size)
{
	/* Align Patch */
	if (!write || pkt_len < 32)
		...
		if (func == SDIO_FUNC_2) {
			sd_err(("%s: [%s] dhd_sdio must align %d bytes"
				...
				__FUNCTION__, write ? "W" : "R", pkt_len,
				blk_size));
		}
			...
			if (need_txglom) {
				if (!localbuf) {
					...
					if (lft_len > prev_lft_len) {
						sd_err(("%s: padding is unexpected! lft_len %d," " prev_lft_len %d %s\n", __FUNCTION__, lft_len, prev_lft_len, write ? "Write" : "Read"));
					}

					localbuf =
					    (uint8 *) MALLOC(sd->osh, lft_len);
					if (localbuf == NULL) {
...
txglomfail:
			...
			if (
				   ...
				   TRUE) {
				pkt_len =
				    sdioh_request_packet_align(pkt_len, write,
							       func, blk_size);
				...
				if (pad > 0) {
					if (func == SDIO_FUNC_2) {
						sd_err(("%s: padding is unexpected! pkt_len %d," " PKTLEN %d lft_len %d %s\n", __FUNCTION__, pkt_len, PKTLEN(sd->osh, pnext), lft_len, write ? "Write" : "Read"));
					}
					if (PKTTAILROOM(sd->osh, pkt) < pad) {
						sd_info(("%s: insufficient tailroom %d, pad %d," " lft_len %d pktlen %d, func %d %s\n", __FUNCTION__, (int)PKTTAILROOM(sd->osh, pkt), pad, lft_len, PKTLEN(sd->osh, pnext), func, write ? "W" : "R"));
						if (PKTPADTAILROOM
						    (sd->osh, pkt, pad)) {
		     ...
		     uint8 * buffer, void *pkt)
{
	...
	if (copylen) {
		...
		if (!write)
			bcopy(PKTDATA(sd->osh, tmppkt), orig_buf,
			      PKTLEN(sd->osh, tmppkt));
...
int sdioh_start(sdioh_info_t * si, int stage)
{
	...
	if (gInstance->func[0]) {
		if (stage == 0) {
			...
			sdio_claim_host(gInstance->func[0]);
			if (gInstance->func[2])
				sdio_claim_irq(gInstance->func[2],
					       IRQHandlerF2);
			if (gInstance->func[1])
				sdio_claim_irq(gInstance->func[1], IRQHandler);
			sdio_release_host(gInstance->func[0]);
