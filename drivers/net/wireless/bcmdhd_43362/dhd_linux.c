...
extern int wl_android_wifictrl_func_add(void);
extern void wl_android_wifictrl_func_del(void);
...
int _dhd_set_mac_address(dhd_info_t * dhd, int ifidx, struct ether_addr *addr)
{
	char buf[32];
	wl_ioctl_t ioc;
	int ret;
		...
		for (i = 0; i < DHD_MAX_IFS; i++) {
			if (dhd->iflist[i]) {
				...
				list_for_each_entry_safe(iter, next,
							 &dhd->iflist[i]->
							 ipv6_list, list) {
					spin_lock_bh(&dhd->iflist[i]->
						     ipv6_lock);
					list_del(&iter->list);
					spin_unlock_bh(&dhd->iflist[i]->
						       ipv6_lock);
					if (iter->ipv6_oper ==
					    DHD_IPV6_ADDR_ADD) {
						ret =
						    dhd_ndo_enable(&dhd->pub,
								   TRUE);
						if (ret < 0) {
					...
					NATIVE_MFREE(dhd->pub.osh, iter,
						     sizeof(struct ipv6_addr));
				}
				if (dhd->set_macaddress == i + 1) {
					dhd->set_macaddress = 0;
					if (_dhd_set_mac_address
					    (dhd, i, &dhd->macvalue) == 0) {
...
#ifdef BCM_FD_AGGR
		bcmerror =
		    dhd_fdaggr_ioctl(pub, ifidx, (wl_ioctl_t *) ioc, buf,
				     buflen);
#else
...
int dhd_deepsleep(dhd_info_t * dhd, int flag)
{
	...
	case 1:		/* Deepsleep on */
		...
		bcm_mkiovar("mpc", (char *)&powervar, 4, iovbuf,
			    sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				 TRUE, 0);

		/* Enable Deepsleep */
		powervar = 1;
		memset(iovbuf, 0, sizeof(iovbuf));
		bcm_mkiovar("deepsleep", (char *)&powervar, 4, iovbuf,
			    sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhdp, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				 TRUE, 0);
		break;
...
dhd_pub_t *dhd_attach(osl_t * osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	dhd =
	    (void *)dhd_os_prealloc(osh, DHD_PREALLOC_DHD_INFO,
				    sizeof(dhd_info_t));
	if (!dhd) {
	...
	bzero(dhd->pub.tcp_ack_info_tbl,
...
int dhd_preinit_ioctls(dhd_pub_t * dhd)
{
	...
	/* Set ack_ratio_depth */
	bcm_mkiovar("ack_ratio_depth", (char *)&ack_ratio_depth, 4, iovbuf,
		    sizeof(iovbuf));
	if ((ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
			      0)) < 0) {
	...
	bcm_mkiovar("ampdu_ba_wsize", (char *)&ampdu_ba_wsize, 4, iovbuf,
		    sizeof(iovbuf));
	if ((ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
			      0)) < 0) {
...
#ifdef SUPPORT_2G_VHT
	bcm_mkiovar("vht_features", (char *)&vht_features, 4, iovbuf,
		    sizeof(iovbuf));
	if ((ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
			      0)) < 0) {
	...
	if (dhd_master_mode) {
		...
		dhd->pktfilter[DHD_MDNS_FILTER_NUM] =
	...
	bcm_mkiovar("ampdu_hostreorder", (char *)&hostreorder, 4, buf,
		    sizeof(buf));
				 ...
				 unsigned long event, void *ptr)
{
	...
	if (event == NETDEV_UP || event == NETDEV_DOWN) {
		_ipv6_addr =
		    NATIVE_MALLOC(dhd_pub->osh, sizeof(struct ipv6_addr));
		if (_ipv6_addr == NULL) {
			DHD_ERROR(("Failed to allocate ipv6\n"));
			goto exit;
		}
		memcpy(&_ipv6_addr->ipv6_addr[0], &inet6_ifa->addr,
		       IPV6_ADDR_LEN);
...
void dhd_free(dhd_pub_t * dhdp)
{
	...
	if (dhdp) {
		...
		if (dhd !=
		    (dhd_info_t *) dhd_os_prealloc(NULL, DHD_PREALLOC_DHD_INFO,
						   0)) {
#endif /* CONFIG_DHD_USE_STATIC_BUF */
			if (dhd)
...
int
dhd_dev_pno_set_for_ssid(struct net_device *dev, wlc_ssid_t * ssids_local,
...
{
	dhd_info_t *dhd = *(dhd_info_t **) netdev_priv(dev);
	return (dhd_pno_set_for_hotlist
		(&dhd->pub, p_pfn_bssid, hotlist_params));
}
...
{
	dhd_info_t *dhd = *(dhd_info_t **) netdev_priv(dev);
	return (dhd_pno_get_for_batch
		(&dhd->pub, buf, bufsize, PNO_STATUS_NORMAL));
}
#endif /* PNO_SUPPORT */
