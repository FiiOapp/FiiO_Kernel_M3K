...
static s32 wl_notify_pfn_status(struct wl_priv *wl, bcm_struct_cfgdev * cfgdev,
				const wl_event_msg_t * e, void *data);
#endif /* PNO_SUPPORT */
static s32 wl_notifier_change_state(struct wl_priv *wl,
				    struct net_info *_net_info,
				    enum wl_status state, bool set);
#ifdef WL_SDO
static s32 wl_svc_resp_handler(struct wl_priv *wl, bcm_struct_cfgdev * cfgdev,
			       const wl_event_msg_t * e, void *data);
static s32 wl_notify_device_discovery(struct wl_priv *wl,
				      bcm_struct_cfgdev * cfgdev,
				      const wl_event_msg_t * e, void *data);
#endif
...
static s32
wl_cfg80211_add_iw_ie(struct wl_priv *wl, struct net_device *ndev, s32 bssidx,
		      s32 pktflag, uint8 ie_id, uint8 * data, uint8 data_len);
#endif /* WL11U */
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if ((err =
	     wldev_ioctl(dev, WLC_GET_BSSID, &bssid, sizeof(bssid), false))) {
							...
							*params)
{
	...
	if (wl->p2p_supported && (wlif_type != -1)) {
		...
		if (wl_get_p2p_status(wl, IF_DELETING)) {
			...
			timeout =
			    wait_event_interruptible_timeout(wl->
							     netif_change_event,
							     (wl_get_p2p_status
							      (wl,
							       IF_DELETING) ==
...
/* Find listen channel */
static s32 wl_find_listen_channel(struct wl_priv *wl, const u8 * ie, u32 ie_len)
{
		...
		if (wl->iscan_on
			...
			if (p2p_ssid) {
				...
				if (!wl->p2p_supported || !p2p_scan(wl)) {
					...
					if (wl_cfgp2p_find_idx
					    (wl, ndev, &bssidx) != BCME_OK) {
						WL_ERR(("Find p2p index from ndev(%p) failed\n", ndev));
						err = BCME_ERROR;
						goto scan_out;
					}
#ifdef WL11U
					if ((interworking_ie =
					     wl_cfg80211_find_interworking_ie((u8 *) request->ie, request->ie_len)) != NULL) {
						err =
						    wl_cfg80211_add_iw_ie(wl,
									  ndev,
									  bssidx,
									  VNDR_IE_CUSTOM_FLAG,
									  interworking_ie->
...
#endif /* WL11U */
					err =
					    wl_cfgp2p_set_management_ie(wl,
									ndev,
									bssidx,
									VNDR_IE_PRBREQ_FLAG,
									(u8 *)
		...
		if (wl->p2p_supported) {
			if (p2p_on(wl) && p2p_scan(wl)) {
				...
				if (wl_get_p2p_status(wl, DISC_IN_PROGRESS)) {
					 ...
					 */
					WL_SD(("P2P_FIND: Discovery offload is in progress." " Do nothing\n"));
					err = -EINVAL;
					goto scan_out;
				}
#endif
...
static void wl_cfg80211_ibss_vsie_free(struct wl_priv *wl)
{
		      ...
		      struct cfg80211_ibss_params *params)
{
	...
	if (IBSS_INITIAL_SCAN_ALLOWED == FALSE) {
		...
		err =
		    wldev_ioctl(dev, WLC_SET_SCANSUPPRESS, &scan_suppress,
				sizeof(int), true);
		if (unlikely(err)) {
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (wl->wdev->wiphy->features & NL80211_FEATURE_FW_4WAY_HANDSHAKE) {
		if (pval == AES_ENABLED)
			err =
			    wldev_iovar_setint_bsscfg(dev, "sup_wpa", 1,
						      bssidx);
		else
			err =
			    wldev_iovar_setint_bsscfg(dev, "sup_wpa", 0,
						      bssidx);
		    ...
		    struct cfg80211_connect_params *sme)
{
	...
	if (!wl_get_drv_status(wl, CONNECTED, dev) &&
	    (ret =
	     wldev_ioctl(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN,
			 false)) == 0) {
	...
	if (chan) {
		...
		WL_DBG(("channel (%d), center_req (%d), %d channels\n",
			wl->channel, chan->center_freq, chan_cnt));
	} else
		wl->channel = 0;
#ifdef BCMWAPI_WPI
	...
	printk("Connectting with " MACDBG
		    ...
		    struct key_params *params)
{
	...
	case WLAN_CIPHER_SUITE_PMK:{
			...
			err =
			    wldev_ioctl(dev, WLC_SET_WSEC_PMK, &pmk,
					sizeof(pmk), true);
			if (err)
				return err;
		}
...
#if defined(RSSIAVG)
		err =
		    wl_update_connected_rssi_cache(dev, &g_rssi2_cache_ctrl,
						   &rssi);
		if (err) {
			WL_ERR(("Could not get rssi (%d)\n", err));
			goto get_station_err;
...
#endif				/* WL_CFG80211_P2P_DEV_IF */
{
	...
	WL_DBG(("Enter, channel: %d, duration ms (%d) SCANNING ?? %s \n",
		...
		duration,
		(wl_get_drv_status(wl, SCANNING, ndev)) ? "YES" : "NO"));
...
static void wl_cfg80211_afx_handler(struct work_struct *work)
{
	...
	if (afx_instance != NULL && wl->afx_hdl->is_active) {
		if (wl->afx_hdl->is_listen && wl->afx_hdl->my_listen_chan) {
			ret = wl_cfgp2p_discover_listen(wl, wl->afx_hdl->my_listen_chan, (100 * (1 + (RANDOM32() % 3))));	/* 100ms ~ 300ms */
		} else {
			      ...
			      u16 action_frame_len, s32 bssidx)
{
	...
	if (config_af_params.search_channel) {
		/* initialize afx_hdl */
		if (wl_cfgp2p_find_idx(wl, dev, &wl->afx_hdl->bssidx) !=
		    BCME_OK) {
			WL_ERR(("Find p2p index from dev(%p) failed\n", dev));
			goto exit;
		}
	...
	ack =
	    wl_cfgp2p_tx_action_frame(wl, dev, af_params,
				      bssidx) ? false : true;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			WL_ERR(("Disconnect STA : %s scb_val.val %d\n",
				bcm_ether_ntoa((const struct ether_addr *)mgmt->
					       da, eabuf), scb_val.val));
...
wl_validate_wpa2ie(struct net_device *dev, bcm_tlv_t * wpa2ie, s32 bssidx)
{
	...
	if ((len -=
	     (WPA_IE_SUITE_COUNT_LEN + (WPA_SUITE_LEN * suite_count))) <= 0)
		return BCME_BADLEN;
				      ...
				      u32 dev_role, s32 bssidx)
{
	...
	if ((info->ssid) && (info->ssid_len > 0) && (info->ssid_len <= 32)) {
		WL_DBG(("SSID (%s) len:%d \n", info->ssid, info->ssid_len));
		if (dev_role == NL80211_IFTYPE_AP) {
			/* Store the hostapd SSID */
			memset(wl->hostapd_ssid.SSID, 0x00, 32);
			memcpy(wl->hostapd_ssid.SSID, info->ssid,
			       info->ssid_len);
		     ...
		     struct net_device *dev, struct cfg80211_ap_settings *info)
{
	...
	if ((err = wl_cfg80211_set_channel(wiphy, dev,
					   dev->ieee80211_ptr->preset_chandef.
					   chan, NL80211_CHAN_HT20) < 0)) {
				 ...
				 struct cfg80211_sched_scan_request *request)
{
	...
	if (ssid_count) {
		if ((ret =
		     dhd_dev_pno_set_for_ssid(dev, ssids_local,
					      request->n_match_sets, pno_time,
			 ...
			 struct regulatory_request *request)
{
	...
	if ((ret = wldev_set_country(wl_to_prmry_ndev(wl), request->alpha2,
				     false,
				     (request->initiator ==
				      NL80211_REGDOM_SET_BY_USER ? true :
				      false))) < 0) {
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev,
			  struct device *sdiofunc_dev, void *data)
{
	s32 err = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)) && \
	(defined(WL_IFACE_COMB_NUM_CHANNELS) || defined(WL_CFG80211_P2P_DEV_IF))
...
static s32 wl_inform_bss(struct wl_priv *wl)
{
	...
	for_each_bss(bss_list, bi, i) {
		if (p2p_disconnected > 0
		    && !memcmp(&bi->BSSID, &p2p_disconnected_bssid,
			       ETHER_ADDR_LEN))
			continue;
		err = wl_inform_single_bss(wl, bi);
	}
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	notif_bss_info->channel =
	    bi->ctl_ch ? bi->
	    ctl_ch : CHSPEC_CHANNEL(wl_chspec_driver_to_host(bi->chanspec));
	...
	WL_SCAN(("BSSID %pM, channel %d, rssi %d, capa 0x04%x, mgmt_type %d, "
		 "frame_len %d, SSID \"%s\"\n", &bi->BSSID,
		 notif_bss_info->channel, notif_bss_info->rssi,
		 mgmt->u.beacon.capab_info, mgmt_type,
			    ...
			    const wl_event_msg_t * e, void *data)
{
	...
	wldev_iovar_getbuf_bsscfg(ndev, "cur_etheraddr",
				  NULL, 0, wl->ioctl_buf, WLC_IOCTL_SMLEN,
			      ...
			      const wl_event_msg_t * e, void *data)
{
	...
	if (event == WLC_E_JOIN || event == WLC_E_START ||
		...
		if (wl_get_drv_status(wl, CONNECTED, ndev)) {
			...
			WL_INFO(("IBSS BSSID is changed from " MACDBG " to "
				 MACDBG "\n", MAC2STRDBG(cur_bssid),
			...
			wl_set_drv_status(wl, CONNECTED, ndev);
			active = true;
			wl_update_prof(wl, ndev, NULL, (void *)&active,
				       WL_PROF_ACT);
		}
	...
	if (wl_get_mode_by_netdev(wl, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(wl, e, ndev)) {
			...
			if (!wl_get_drv_status(wl, DISCONNECTING, ndev)) {
				...
				dhd_conf_set_phyoclscdenable((dhd_pub_t *) wl->
							     pub);
				WL_DBG(("joined in BSS network \"%s\"\n",
					((struct wlc_ssid *)
					 wl_read_prof(wl, ndev,
						      WL_PROF_SSID))->SSID));
			}
			...
			if (wl_get_drv_status(wl, CONNECTED, ndev)) {
				...
				printk
				    ("link down if %s may call cfg80211_disconnected. "
				     ...
				     MAC2STRDBG((u8 *) (&e->addr)));
				if (memcmp(curbssid, &e->addr, ETHER_ADDR_LEN)
				    != 0) {
				...
				if (!memcmp
				    (ndev->name, WL_P2P_INTERFACE_PREFIX,
				     strlen(WL_P2P_INTERFACE_PREFIX))) {
					// terence 20130703: Fix for wrong group_capab (timing issue)
					p2p_disconnected = 1;
					memcpy(&p2p_disconnected_bssid,
					       curbssid, ETHER_ADDR_LEN);
				}
				wl_clr_drv_status(wl, CONNECTED, ndev);
				if (!wl_get_drv_status(wl, DISCONNECTING, ndev)) {
...
void wl_stop_wait_next_action_frame(struct wl_priv *wl)
{
			...
			const wl_event_msg_t * e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
...
#ifdef WL_SDO
			if (wl_get_p2p_status(wl, DISC_IN_PROGRESS)) {
				WL_ERR(("SD offload is in progress. Don't report the" "frame via rx_mgmt path\n"));
				goto exit;
			}
#endif
		...
		if (event == WLC_E_P2P_PROBREQ_MSG) {
			 ...
			 * GO-NEG Phase
			 */
			if (wl->p2p && wl_get_p2p_status(wl, GO_NEG_PHASE)) {
...
static void wl_init_escan_result_buf(struct wl_priv *wl)
{
	wl->escan_info.escan_buf =
	    dhd_os_prealloc(NULL, DHD_PREALLOC_WIPHY_ESCAN0, 0);
	bzero(wl->escan_info.escan_buf, ESCAN_BUF_SIZE);
}

static void wl_deinit_escan_result_buf(struct wl_priv *wl)
{
			    ...
			    const wl_event_msg_t * e, void *data)
{
	...
	if (!ndev || !wl->escan_on ||
		...
		WL_ERR(("escan is not ready ndev %p wl->escan_on %d"
			...
			ndev, wl->escan_on, wl_get_drv_status(wl, SCANNING,
							      ndev),
			ntoh32(e->event_type), ntoh32(e->status)));
		goto exit;
	}
				    ...
				    enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (wl->vsdb_mode || _net_info->pm_block) {
			...
			for_each_ndev(wl, iter, next) {
				if ((err =
				     wldev_ioctl(iter->ndev, WLC_SET_PM, &pm,
						 sizeof(pm), true)) != 0) {
					if (err == -ENODEV)
	...
	while (down_interruptible(&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			...
			if ((wl_is_p2p_event(e) == TRUE) && (wl->p2p_wdev)) {
				...
				cfgdev =
				    ndev_to_wdev(dhd_idx2net
						 ((struct dhd_pub *)(wl->pub),
						  e->emsg.ifidx));
			}
#elif defined(WL_ENABLE_P2P_IF)
			...
			if (e->etype < WLC_E_LAST && wl->evt_handler[e->etype]) {
				wl->evt_handler[e->etype] (wl, cfgdev, &e->emsg,
							   e->edata);
			} else {
				   ...
				   u32 * ie_size)
{
	...
	if (ssidie[1] != bi->SSID_len) {
		...
		memmove(ssidie + bi->SSID_len + 2, ssidie + 2,
			*ie_size - (ssidie + 2 - ie_stream));
			   ...
			   const wl_event_msg_t * e, void *data)
{
	...
	if ((event != WLC_E_P2PO_ADD_DEVICE)
	    && (event != WLC_E_P2PO_DEL_DEVICE)) {
	...
	if (event == WLC_E_P2PO_DEL_DEVICE) {
		WL_SD(("DEV_LOST MAC:" MACDBG " \n",
		       MAC2STRDBG(e->addr.octet)));
		err =
		    wl_genl_send_msg(ndev, event, (u8 *) e->addr.octet,
				     ETH_ALEN, 0, 0);
	} else {
...
s32 wl_cfg80211_sdo_deinit(struct wl_priv * wl)
{
	...
	if (wl->sdo->dd_state &&
	    (ret = wldev_iovar_setbuf_bsscfg(wl_to_prmry_ndev(wl),
					     "p2po_stop", (void *)&sdo_pause,
					     sizeof(sdo_pause), wl->ioctl_buf,
...
s32 wl_cfg80211_resume_sdo(struct net_device * dev, struct wl_priv * wl)
{
	...
	if (wl->sdo->dd_state == WL_DD_STATE_SEARCH) {
		...
		if ((ret =
		     wldev_iovar_setbuf_bsscfg(dev, "p2po_listen",
					       (void *)&sd_listen,
					       sizeof(wl_sd_listen_t),
					       wl->ioctl_buf, WLC_IOCTL_SMLEN,
					       bssidx, NULL)) < 0) {
		    ...
		    const wl_event_msg_t * e, void *data)
{
	...
	if (event == WLC_E_SERVICE_FOUND) {
		...
		tot_len =
		    data_len - (sizeof(wl_event_sd_t) - sizeof(wl_sd_tlv_t));

		WL_SD(("WLC_E_SERVICE_FOUND " MACDBG
		       " data_len:%d tlv_count:%d \n", MAC2STRDBG(dst_mac),
		       data_len, sdo_hdr.count));
...
s32 wl_sd_handle_sd_req(struct net_device * dev, u8 * buf, int len)
{
	...
	if ((ret =
	     wldev_iovar_setbuf_bsscfg(dev, "p2po_sd_req_resp", (void *)sdreq,
...
s32 wl_sd_handle_sd_stop_discovery(struct net_device * dev, u8 * buf, int len)
{
	...
	if ((ret =
	     wldev_iovar_setbuf_bsscfg(dev, "p2po_stop", (void *)&sdo_pause,
				       sizeof(sdo_pause), wl->ioctl_buf,
				       WLC_IOCTL_SMLEN, bssidx,
				       &wl->ioctl_buf_sync)) < 0) {
		WL_ERR(("p2po_stop Failed :%d\n", ret));
...
s32 wl_sd_handle_sd_find(struct net_device * dev, u8 * buf, int len)
{
	...
	if (buf && strncmp(buf, "chan=", strlen("chan=")) == 0) {
		...
		if ((ret =
		     wldev_iovar_setbuf_bsscfg(dev, "p2po_listen_channel",
					       (void *)&channel,
					       sizeof(channel), wl->ioctl_buf,
					       WLC_IOCTL_SMLEN, bssidx,
					       &wl->ioctl_buf_sync)) < 0) {
...
s32 wl_sd_handle_sd_listen(struct net_device * dev, u8 * buf, int len)
{
	...
	if ((ret =
	     wldev_iovar_setbuf_bsscfg(dev, "p2po_listen_channel",
				       ...
				       wl->ioctl_buf, WLC_IOCTL_SMLEN, bssidx,
				       &wl->ioctl_buf_sync)) < 0) {
		WL_ERR(("p2po_listen_channel Failed :%d\n", ret));
		return -1;
	}
	...
	if ((ret =
	     wldev_iovar_setbuf_bsscfg(dev, "p2po_listen", (void *)&sd_listen,
				       sizeof(wl_sd_listen_t), wl->ioctl_buf,
				       WLC_IOCTL_SMLEN, bssidx,
				       &wl->ioctl_buf_sync)) < 0) {
		WL_ERR(("p2po_listen Failed :%d\n", ret));
		return -1;
			   ...
			   int len)
{
	...
	if (strncmp(cmd, "P2P_SD_REQ", strlen("P2P_SD_REQ")) == 0) {
	    ...
	    if (strncmp(cmd, "P2P_SD_CANCEL_REQ", strlen("P2P_SD_CANCEL_REQ"))
		== 0) {
		ret = wl_sd_handle_sd_cancel_req(dev, buf);
	} else if (strncmp(cmd, "P2P_SD_SVC_ADD", strlen("P2P_SD_SVC_ADD")) ==
		   0) {
		ret = wl_sd_handle_sd_add_svc(dev, buf, len);
	} else if (strncmp(cmd, "P2P_SD_SVC_DEL", strlen("P2P_SD_SVC_DEL")) ==
		   0) {
			     ...
			     int *channel)
{
	...
	while (retry--) {
		...
		if ((ret == 0) && (dtoh32(chosen) != 0)) {
			if (dhd_bus_chip_id(bcmsdh_get_drvdata()) !=
			    BCM43362_CHIP_ID
			    && dhd_bus_chip_id(bcmsdh_get_drvdata()) !=
			    BCM4330_CHIP_ID) {
				u32 chanspec = 0;
				int ctl_chan;
...
wl_cfg80211_get_best_channels(struct net_device * dev, char *cmd, int total_len)
{
	...
	ret =
	    wldev_ioctl(dev, WLC_GET_BAND, &band_cur, sizeof(band_cur), false);
...
void get_primary_mac(struct wl_priv *wl, struct ether_addr *mac)
{
	wldev_iovar_getbuf_bsscfg(wl_to_prmry_ndev(wl), "cur_etheraddr", NULL,
				  0, wl->ioctl_buf, WLC_IOCTL_SMLEN, 0,
				  &wl->ioctl_buf_sync);
...
s32 wl_cfg80211_set_band(struct net_device * ndev, int band)
{
	...
	if ((ret = wldev_iovar_setbuf(ndev, "roam_band", &band,
				      sizeof(int), ioctl_buf, sizeof(ioctl_buf),
				      NULL)) < 0) {
	...
	if (wl->scan_suppressed) {
		if (ret) {
			...
			mod_timer(&wl->scan_supp_timer,
				  jiffies +
				  msecs_to_jiffies(WL_SCAN_SUPPRESS_RETRY));
		} else {
			WL_DBG(("Start wlan_timer to clear of scan_suppress \n"));
			mod_timer(&wl->scan_supp_timer,
				  jiffies +
				  msecs_to_jiffies(WL_SCAN_SUPPRESS_TIMEOUT));
		}
	}
exit:
	return ret;
