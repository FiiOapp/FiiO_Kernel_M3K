	...
	if (!strncmp(pos, PNO_BATCHING_SET, strlen(PNO_BATCHING_SET))) {
		...
		while ((token = strsep(&pos, PNO_PARAMS_DELIMETER)) != NULL) {
			...
			if (!strncmp
				...
				if (tokens != 1) {
					err = BCME_ERROR;
					ANDROID_ERROR(("%s : invalid format for channel" " <> params\n", __FUNCTION__));
					goto exit;
				}
				    ...
				    int total_len)
{
	...
	res =
	    dhd_dev_pno_set_for_ssid(dev, ssids_local, nssid, pno_time,
		...
		if (assoc_maclist->count) {
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				for (j = 0; j < maclist->count; j++) {
					ANDROID_INFO(("%s : associated=" MACDBG
						      " list=" MACDBG "\n",
						      __FUNCTION__,
				 ...
				 int total_len)
{
	...
	bcm_strncpy_s(fw_path, sizeof(fw_path),
		      command + strlen(CMD_SETFWPATH) + 1,
		      MOD_PARAM_PATHLEN - 1);
...
wl_android_set_pmk(struct net_device *dev, char *command, int total_len)
{
	...
	error =
	    wldev_iovar_setbuf(dev, "okc_info_pmk", pmk, 32, smbuf,
			       sizeof(smbuf), NULL);
	if (error) {
				       ...
				       int total_len)
{
	...
	err =
	    wldev_iovar_setbuf(dev, "ie", vndr_ie, tot_len, ioctl_buf,
...
int wl_android_get_channel(struct net_device *dev, char *command, int total_len)
{
	int ret;
	channel_info_t ci;
	int bytes_written = 0;
	...
	if (!
	    (ret =
	     wldev_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(channel_info_t),
			 FALSE))) {
		ANDROID_TRACE(("hw_channel %d\n", ci.hw_channel));
		ANDROID_TRACE(("target_channel %d\n", ci.target_channel));
		ANDROID_TRACE(("scan_channel %d\n", ci.scan_channel));
		bytes_written =
		    snprintf(command, sizeof(channel_info_t) + 2, "channel %d",
			     ci.hw_channel);
...
int
wl_android_get_roam_trigger(struct net_device *dev, char *command,
			    int total_len)
{
	int ret;
	int bytes_written;
...
wl_android_get_keep_alive(struct net_device * dev, char *command, int total_len)
{
	...
	if ((res =
	     wldev_ioctl(dev, WLC_GET_VAR, mkeep_alive_pktp, len, FALSE)) < 0) {
	...
	bytes_written =
	    snprintf(command, total_len, "mkeep_alive_period_msec %d ",
		     dtoh32(mkeep_alive_pktp->period_msec));
	bytes_written += snprintf(command + bytes_written, total_len, "0x");
	for (i = 0; i < mkeep_alive_pktp->len_bytes; i++) {
		bytes_written +=
		    snprintf(command + bytes_written, total_len, "%x",
...
int wl_android_set_pm(struct net_device *dev, char *command, int total_len)
{
	int pm, ret = -1;
...
int wl_android_get_pm(struct net_device *dev, char *command, int total_len)
{
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	if (copy_from_user
	    (&priv_cmd, ifr->ifr_data, sizeof(android_wifi_priv_cmd))) {
		ret = -EFAULT;
		goto exit;
	}
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		bytes_written =
		 ...
		 (command, CMD_RXFILTER_START,
		  strlen(CMD_RXFILTER_START)) == 0) {
		bytes_written = net_os_enable_packet_filter(net, 1);
	} else
	    if (strnicmp(command, CMD_RXFILTER_STOP, strlen(CMD_RXFILTER_STOP))
		== 0) {
		...
		bytes_written =
		 ...
		 (command, CMD_BTCOEXSCAN_START,
		  strlen(CMD_BTCOEXSCAN_START)) == 0) {
		/* TBD: BTCOEXSCAN-START */
	} else
	    if (strnicmp
		(command, CMD_BTCOEXSCAN_STOP,
	    ...
	    if (strnicmp(command, CMD_SETSUSPENDOPT, strlen(CMD_SETSUSPENDOPT))
		== 0) {
		bytes_written =
		    wl_android_set_suspendopt(net, command, priv_cmd.total_len);
	} else
	    if (strnicmp
		(command, CMD_SETSUSPENDMODE,
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		bytes_written =
		    wldev_set_country(net, country_code, true, true);
	}
#endif /* WL_CFG80211 */

#ifdef PNO_SUPPORT
	else if (strnicmp
		 (command, CMD_PNOSSIDCLR_SET,
...
#if defined(WL_SDO) && defined(WL_CFG80211)
	else if (strnicmp
		 (command, CMD_P2P_SD_OFFLOAD,
	...
	} else if (strnicmp(command, CMD_SETCCKM_KRK, strlen(CMD_SETCCKM_KRK))
		   == 0) {
		...
		bytes_written = wl_cfg80211_get_best_channels(net, command,
		 ...
		 (command, CMD_HAPD_MAC_FILTER,
		  strlen(CMD_HAPD_MAC_FILTER)) == 0) {
		int skip = strlen(CMD_HAPD_MAC_FILTER) + 1;
		wl_android_set_mac_address_filter(net,
						  (const char *)command + skip);
	} else if (strnicmp(command, CMD_SETROAMMODE, strlen(CMD_SETROAMMODE))
		   == 0)
		bytes_written =
		    wl_android_set_roam_mode(net, command, priv_cmd.total_len);
#ifdef WL_CFG80211
	else if (strnicmp(command, CMD_MIRACAST, strlen(CMD_MIRACAST)) == 0)
		bytes_written =
		    wl_android_set_miracast(net, command, priv_cmd.total_len);
	else if (strnicmp
		 (command, CMD_SETIBSSBEACONOUIDATA,
		  strlen(CMD_SETIBSSBEACONOUIDATA)) == 0)
		bytes_written =
		    wl_android_set_ibss_beacon_ouidata(net, command,
						       priv_cmd.total_len);
#endif
	else if (strnicmp(command, CMD_GET_CHANNEL, strlen(CMD_GET_CHANNEL)) ==
		 0) {
		bytes_written =
		    wl_android_get_channel(net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SET_ROAM, strlen(CMD_SET_ROAM)) == 0) {
		bytes_written =
		    wl_android_set_roam_trigger(net, command,
						priv_cmd.total_len);
	} else if (strnicmp(command, CMD_GET_ROAM, strlen(CMD_GET_ROAM)) == 0) {
		bytes_written =
		    wl_android_get_roam_trigger(net, command,
						priv_cmd.total_len);
	} else
	    if (strnicmp
		(command, CMD_SET_KEEP_ALIVE,
		 strlen(CMD_SET_KEEP_ALIVE)) == 0) {
		int skip = strlen(CMD_SET_KEEP_ALIVE) + 1;
		bytes_written =
		    wl_android_set_keep_alive(net, command + skip,
					      priv_cmd.total_len - skip);
	} else
	    if (strnicmp
		(command, CMD_GET_KEEP_ALIVE,
		 strlen(CMD_GET_KEEP_ALIVE)) == 0) {
		int skip = strlen(CMD_GET_KEEP_ALIVE) + 1;
		bytes_written =
		    wl_android_get_keep_alive(net, command + skip,
					      priv_cmd.total_len - skip);
	} else if (strnicmp(command, CMD_GET_PM, strlen(CMD_GET_PM)) == 0) {
		bytes_written =
		    wl_android_get_pm(net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SET_PM, strlen(CMD_SET_PM)) == 0) {
		bytes_written =
		    wl_android_set_pm(net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_MONITOR, strlen(CMD_MONITOR)) == 0)
		bytes_written =
		    wl_android_set_monitor(net, command, priv_cmd.total_len);
	else {
...
s32
wl_genl_send_msg(struct net_device * ndev,
		 u32 event_type, u8 * buf, u16 len, u8 * subhdr, u16 subhdr_len)
{
	int ret = 0;
	struct sk_buff *skb;
	void *msg;
	...
	if (mcast) {
		...
		if ((err =
		     genlmsg_multicast(skb, 0, wl_genl_mcast.id,
				       GFP_ATOMIC)) < 0)
			       ...
			       int *rssi_avg)
{
	...
	if (timeout.tv_sec < now.tv_sec) {
		 ...
		 */
		ANDROID_TRACE(("%s: Too long timeout (secs=%d) to ever happen - now=%lu, timeout=%lu", __FUNCTION__, RSSICACHE_TIMEOUT, now.tv_sec, timeout.tv_sec));
	}
...
void wl_delete_dirty_bss_cache(wl_bss_cache_ctrl_t * bss_cache_ctrl)
{
	...
	for (; node;) {
		...
		if (now.tv_sec > node->tv.tv_sec) {
			...
			ANDROID_TRACE(("%s: Del %d with BSSID %pM, RSSI=%d, SSID \"%s\"\n", __FUNCTION__, i, &node->results.bss_info->BSSID, dtoh16(node->results.bss_info->RSSI), node->results.bss_info->SSID));
			kfree(node);
			if (tmp == 1) {
				node = *bss_head;
				prev = node;
				 ...
				 u8 * bssid)
{
	...
	for (; node;) {
		i++;
		if (!memcmp
		    (&node->results.bss_info->BSSID, bssid, ETHER_ADDR_LEN)) {
			if (node == *bss_head) {
				tmp = 1;
				*bss_head = node->next;
			} else {
		    ...
		    wl_scan_results_t * ss_list)
{
	...
	if (timeout.tv_sec < now.tv_sec) {
		 ...
		 * to be infinite, i.e., the timeout would never happen.
		 */
		ANDROID_TRACE(("%s: Too long timeout (secs=%d) to ever happen - now=%lu, timeout=%lu", __FUNCTION__, BSSCACHE_TIMEOUT, now.tv_sec, timeout.tv_sec));
	}
	...
	for (i = 0; i < ss_list->count; i++) {
		...
		for (; node;) {
			if (!memcmp
			    (&node->results.bss_info->BSSID, &bi->BSSID,
			     ETHER_ADDR_LEN)) {
				tmp = node;
				leaf =
				    kmalloc(dtoh32(bi->length) +
				...
				memcpy(leaf->results.bss_info, bi,
				       dtoh32(bi->length));
				leaf->next = node->next;
				leaf->dirty = 0;
				leaf->tv = timeout;
		...
		leaf =
		    kmalloc(dtoh32(bi->length) +
			    WLC_IW_SS_CACHE_CTRL_FIELD_MAXLEN, GFP_KERNEL);
		if (!leaf) {
			ANDROID_ERROR(("%s: Memory alloc failure %d\n",
				       __FUNCTION__,
				       dtoh32(bi->length) +
