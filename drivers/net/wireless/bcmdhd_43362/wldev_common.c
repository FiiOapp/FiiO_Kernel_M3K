...
int wldev_set_band(struct net_device *dev, uint band)
{
	...
	if ((band == WLC_BAND_AUTO) || (band == WLC_BAND_5G)
	    || (band == WLC_BAND_2G)) {
		error =
		    wldev_ioctl(dev, WLC_SET_BAND, &band, sizeof(band), true);
		if (!error)
		      ...
		      bool user_enforced)
{
	...
	error =
	    wldev_iovar_getbuf(dev, "country", NULL, 0, &cspec, sizeof(cspec),
			       NULL);
	if (error < 0) {
	...
	if ((error < 0) ||
		...
		get_customized_country_code((char *)&cspec.country_abbrev,
					    &cspec);
		error =
		    wldev_iovar_setbuf(dev, "country", &cspec, sizeof(cspec),
