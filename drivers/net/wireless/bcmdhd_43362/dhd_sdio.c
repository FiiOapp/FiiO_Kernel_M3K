...
static int dhd_bcmsdh_send_buf(dhd_bus_t * bus, uint32 addr, uint fn,
			       ...
			       bcmsdh_cmplt_fn_t complete, void *handle);
#ifdef BCMSDIOH_TXGLOM
static void dhd_bcmsdh_glom_post(dhd_bus_t * bus, uint8 * frame, void *pkt,
				 uint len);
...
static void dhd_overflow_war(struct dhd_bus *bus)
{
	int err;
	uint8 devctl, wm, mes;
	...
	DHD_INFO(("Apply overflow WAR: 0x%02x 0x%02x 0x%02x\n",
		  bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL,
				  &err), bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
							 SBSDIO_WATERMARK,
							 &err),
		  bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
				  SBSDIO_FUNC1_MESBUSYCTRL, &err)));
}
...
static bool dhdsdio_sr_cap(dhd_bus_t * bus)
{
	...
	if (bus->sih->chip == BCM4324_CHIP_ID) {
		...
		if ((bus->sih->chip == BCM4350_CHIP_ID) || 0)
		...
		data = bcmsdh_reg_read(bus->sdh,
				       SI_ENUM_BASE + OFFSETOF(chipcregs_t,
							       retention_ctl),
				       4);
		if ((data &
		     (RCTL_MACPHY_DISABLE_MASK | RCTL_LOGIC_DISABLE_MASK)) == 0)
			cap = TRUE;
	}
...
static int dhdsdio_srwar_init(dhd_bus_t * bus)
{
...
static int dhdsdio_sr_init(dhd_bus_t * bus)
{
	...
	val =
	    bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WAKEUPCTRL,
			    NULL);
...
static int dhdsdio_clk_kso_init(dhd_bus_t * bus)
{
	 ...
	 */
	val =
	    bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, NULL);
...
static int dhdsdio_devcap_set(dhd_bus_t * bus, uint8 cap)
{
	...
	bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_BRCM_CARDCAP, cap,
			 &err);
	if (err)
...
static int dhdsdio_bussleep(dhd_bus_t * bus, bool sleep)
{
	...
	if (sleep) {
		...
		if (!SLPAUTO_ENAB(bus)) {
			 ...
			 */
			err =
			    dhdsdio_clk_devsleep_iovar(bus, TRUE /* sleep */ );
		}
		...
		if (!SLPAUTO_ENAB(bus)) {
			...
			if (retries <= retry_limit)
				W_SDREG(SMB_DEV_INT, &regs->tosbmailbox,
					retries);
			...
			err =
			    dhdsdio_clk_devsleep_iovar(bus, FALSE /* wake */ );
		}
...
void dhd_onoff_tcpack_sup(void *pub, bool on)
{
	...
	if (dhd_use_tcpack_suppress != on) {
		...
		bzero(dhdp->tcp_ack_info_tbl,
		      sizeof(struct tcp_ack_info) * MAXTCPSTREAMS);
		...
		if (tcp_ack_info->p_tcpackinqueue == pkt) {
			...
			if (i < tbl_cnt - 1) {
				memmove(&dhdp->tcp_ack_info_tbl[i],
					...
					sizeof(struct tcp_ack_info) * (tbl_cnt -
								       (i +
									1)));
			}
			bzero(&dhdp->tcp_ack_info_tbl[tbl_cnt - 1],
			      sizeof(struct tcp_ack_info));
			if (--dhdp->tcp_ack_info_cnt < 0) {
				DHD_ERROR(("dhdsdio_sendfromq:(ERROR) tcp_ack_info_cnt %d" " Stop using tcpack_suppress\n", dhdp->tcp_ack_info_cnt));
...
bool dhd_tcpack_suppress(dhd_pub_t * dhdp, void *pkt)
{
	...
	if (tcp_header[13] == 0x10) {
		...
		uint16 ip_tcp_ttllen =
		    (ip_header[3] & 0xff) + (ip_header[2] << 8);
		...
		if (ip_tcp_ttllen == ip_hdr_len + tcp_hdr_len) {
			...
			for (i = 0; i < dhdp->tcp_ack_info_cnt; i++) {
				if (dhdp->tcp_ack_info_tbl[i].p_tcpackinqueue &&
				    !memcmp(&ip_header[12],
					    dhdp->tcp_ack_info_tbl[i].ipaddrs,
					    8)
				    && !memcmp(tcp_header,
					       dhdp->tcp_ack_info_tbl[i].
					       tcpports, 4)) {
					tcp_ack_info =
					    &dhdp->tcp_ack_info_tbl[i];
					break;
			...
			if (i == dhdp->tcp_ack_info_cnt && i < MAXTCPSTREAMS)
				tcp_ack_info =
				    &dhdp->tcp_ack_info_tbl[dhdp->
							    tcp_ack_info_cnt++];

			if (!tcp_ack_info) {
				DHD_TRACE(("dhd_tcpack_suppress: No empty tcp ack info" "%d %d %d %d, %d %d %d %d\n", tcp_header[0], tcp_header[1], tcp_header[2], tcp_header[3], dhdp->tcp_ack_info_tbl[i].tcpports[0], dhdp->tcp_ack_info_tbl[i].tcpports[1], dhdp->tcp_ack_info_tbl[i].tcpports[2], dhdp->tcp_ack_info_tbl[i].tcpports[3]));
				dhd_os_tcpackunlock(dhdp);
				return FALSE;
			}
			...
			if (tcp_ack_info->p_tcpackinqueue) {
				if (tcp_ack_num > tcp_ack_info->tcpack_number) {
					...
					if ((ip_tcp_ttllen ==
						...
						DHD_TRACE(("dhd_tcpack_suppress: pkt 0x%p" " TCP ACK replace %ud -> %ud\n", prevpkt, tcp_ack_info->tcpack_number, tcp_ack_num));
						tcp_ack_info->tcpack_number =
						    tcp_ack_num;
						dhd_os_tcpackunlock(dhdp);
						return TRUE;
					} else
						DHD_TRACE(("dhd_tcpack_suppress: len mismatch" " %d(%d) %d(%d)\n", PKTLEN(dhdp->osh, pkt), ip_tcp_ttllen, PKTLEN(dhdp->osh, prevpkt), tcp_ack_info->ip_tcp_ttllen));
				} else {
#ifdef TCPACK_TEST
					void *prevpkt =
					    tcp_ack_info->p_tcpackinqueue;
#endif
					DHD_TRACE(("dhd_tcpack_suppress: TCP ACK number reverse" " prev %ud (0x%p) new %ud (0x%p)\n", tcp_ack_info->tcpack_number, tcp_ack_info->p_tcpackinqueue, tcp_ack_num, pkt));
#ifdef TCPACK_TEST
					if (PKTLEN(dhdp->osh, pkt) ==
					    PKTLEN(dhdp->osh, prevpkt)) {
						PKTFREE(dhdp->osh, pkt, FALSE);
						dhd_os_tcpackunlock(dhdp);
						return TRUE;
	      ...
	      bool queue_only)
{
	...
	*(((uint16 *) frame) + 1) = htol16(~len);
	...
	if (bus->glom_enable) {
		...
		swheader =
		    ((chan << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK) |
		    ((bus->tx_seq +
		      bus->glom_cnt) % SDPCM_SEQUENCE_WRAP) | (((pad1 +
								 SDPCM_HDRLEN)
								<<
								SDPCM_DOFFSET_SHIFT)
							       &
							       SDPCM_DOFFSET_MASK);
		htol32_ua_store(swheader,
				frame + SDPCM_FRAMETAG_LEN + SDPCM_HWEXT_LEN);
		htol32_ua_store(0,
				frame + SDPCM_FRAMETAG_LEN + SDPCM_HWEXT_LEN +
				sizeof(swheader));
		...
		if (queue_only) {
			...
			htol32_ua_store(hwheader2,
					frame + SDPCM_FRAMETAG_LEN + 4);
			...
			if (PKTTAILROOM(osh, pkt) < real_pad) {
				DHD_INFO(("%s 2: insufficient tailroom %d"
					  " for %d real_pad\n",
					  __FUNCTION__, (int)PKTTAILROOM(osh,
									 pkt),
					  real_pad));
				if (PKTPADTAILROOM(osh, pkt, real_pad)) {
					DHD_ERROR(("CHK2: padding error size %d." " %d more pkts are discarded together.\n", real_pad, bus->glom_cnt));
					/* Save the pkt pointer in bus glom array
			...
			frame_tmp =
			    (uint8 *) PKTDATA(osh, bus->glom_pkt_arr[0]);
			*(uint16 *) frame_tmp = htol16(bus->glom_total_len);
			*(((uint16 *) frame_tmp) + 1) =
			    htol16(~bus->glom_total_len);
		}
	} else
#endif /* BCMSDIOH_TXGLOM */
	{
		...
		htol32_ua_store(0,
				frame + SDPCM_FRAMETAG_LEN + sizeof(swheader));
	...
	do {
		...
		if (ret == 0) {
			...
			if (bus->glom_enable) {
				bus->tx_seq =
				    (bus->tx_seq +
				     bus->glom_cnt) % SDPCM_SEQUENCE_WRAP;
			} else
#endif
			{
				bus->tx_seq =
	...
	while (pkt_cnt) {
		...
		if (bus->glom_enable) {
			...
			if (!queue_only)
				pkt =
				    bus->glom_pkt_arr[bus->glom_cnt - pkt_cnt];

			frame = (uint8 *) PKTDATA(osh, pkt);
			doff =
			    ltoh32_ua(frame + SDPCM_FRAMETAG_LEN +
				      SDPCM_HWEXT_LEN);
			doff =
			    (doff & SDPCM_DOFFSET_MASK) >> SDPCM_DOFFSET_SHIFT;
#ifdef BCMLXSDMMC
...
static uint dhdsdio_sendfromq(dhd_bus_t * bus, uint maxframes)
{
	...
	for (cnt = 0; (cnt < maxframes) && DATAOK(bus); cnt++) {
		...
		if (bus->glom_enable) {
			...
			glom_cnt =
			    MIN(glom_cnt, pktq_mlen(&bus->txq, tx_prec_map));
			...
			for (i = 0; i < glom_cnt; i++) {
				...
				datalen_tmp =
				    (PKTLEN(bus->dhd->osh, pkt) - SDPCM_HDRLEN);
...
#ifndef SDTEST
			ret =
			    dhdsdio_txpkt(bus, pkt, SDPCM_DATA_CHANNEL, TRUE,
					  FALSE);
#else
			ret = dhdsdio_txpkt(bus,
					    pkt,
...
static void dhdsdio_sendpendctl(dhd_bus_t * bus)
{
	...
	ret =
	    dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
				...
				(uint32) bus->ctrl_frame_len, NULL, NULL, NULL);
	ASSERT(ret != BCME_PENDING);
	if (ret == BCME_NODEVICE) {
		DHD_ERROR(("%s: Device asleep already\n", __FUNCTION__));
	} else if (ret < 0) {
	...
	if (bus->glom_enable) {
		...
		swheader =
		    ((SDPCM_CONTROL_CHANNEL << SDPCM_CHANNEL_SHIFT) &
		     SDPCM_CHANNEL_MASK)
		    | bus->
		    tx_seq | ((doff << SDPCM_DOFFSET_SHIFT) &
			      SDPCM_DOFFSET_MASK);
		htol32_ua_store(swheader,
				frame + SDPCM_FRAMETAG_LEN + SDPCM_HWEXT_LEN);
		htol32_ua_store(0,
...
static int dhdsdio_readshared(dhd_bus_t * bus, sdpcm_shared_t * sh)
{
	...
	do {
		...
		if ((rv =
		     dhdsdio_membytes(bus, FALSE, shaddr, (uint8 *) & addr,
				      4)) < 0)
			return rv;
	...
	*bcmerror = 0;
	...
	if (bus->sih->chip == BCM4330_CHIP_ID) {
		uart_enab = CC_PLL_CHIPCTRL_SERIAL_ENAB;
		   ...
		   bus->sih->chip == BCM43341_CHIP_ID || 0) {
		if (enable) {
		...
		int val_size)
{
	...
	if ((vi->varid == IOV_KSO) && (IOV_ISSET(actionid))) {
	...
	} else if ((vi->varid == IOV_DEVSLEEP) && (IOV_ISSET(actionid))) {
		{
			...
			if (!SLPAUTO_ENAB(bus) && (bool_val == FALSE)
			    && (bus->ipend)) {
	...
	case IOV_GVAL(IOV_MEMBYTES):
		{
			...
			if (si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
				...
				if ((bus->orig_ramsize) &&
					...
					} else if (REMAP_ENAB(bus)
						   && REMAP_ISADDR(bus, address)
						   && remap) {
	...
	case IOV_SVAL(IOV_WATERMARK):
		...
		watermark =
		    (watermark >
		     SBSDIO_WATERMARK_MASK) ? SBSDIO_WATERMARK_MASK : watermark;
	...
	case IOV_SVAL(IOV_TXGLOMMODE):
		if ((int_val != SDPCM_TXGLOM_CPY)
		    && (int_val != SDPCM_TXGLOM_MDESC)) {
			bcmerror = BCME_RANGE;
		} else {
			if ((bus->glom_mode =
			     bcmsdh_set_mode(bus->sdh,
					     (uint) int_val)) != int_val)
				bcmerror = BCME_ERROR;
		}
		break;
#endif /* BCMSDIOH_TXGLOM */
...
static int dhdsdio_download_state(dhd_bus_t * bus, bool enter)
{
	...
	if (enter) {
		...
		if (!foundcr4) {
			...
			if (REMAP_ENAB(bus)
			    && si_socdevram_remap_isenb(bus->sih))
...
void dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		if (KSO_ENAB(bus)) {
			...
			W_SDREG(local_hostintmask, &bus->regs->intstatus,
				retries);
		}
...
void dhd_txglom_enable(dhd_pub_t * dhdp, bool enable)
{
	...
	if (enable) {
		...
		ret = dhd_wl_ioctl_cmd(dhdp,
				       WLC_SET_VAR, buf, sizeof(buf), TRUE, 0);
		if (!(ret < 0)) {
...
static uint8 dhdsdio_rxglom(dhd_bus_t * bus, uint8 rxseq)
{
	...
	if (bus->glom) {
		...
		if ((uint16) ~ (sublen ^ check)) {
			...
			DHD_ERROR(("%s (superframe): Bad data offset %d: HW %d pkt %d min %d\n", __FUNCTION__, doff, sublen, PKTLEN(osh, pfirst), SDPCM_HDRLEN_RX));
			errcode = -1;
		}
		...
		for (num = 0; pfirst; rxseq++, pfirst = pnext) {
			...
			if (PKTLEN(osh, pfirst) == 0) {
			    ...
			    if (dhd_prot_hdrpull
				(bus->dhd, &ifidx, pfirst, reorder_info_buf,
				 &reorder_info_len) != 0) {
			...
			if (reorder_info_len) {
				...
				dhd_process_pkt_reorder_info(bus->dhd,
							     reorder_info_buf,
							     reorder_info_len,
							     &ppfirst,
							     &free_buf_count);
				...
				if (list_tail[ifidx] == NULL) {
					list_head[ifidx] = list_tail[ifidx] =
					    pfirst;
				} else {
					PKTSETNEXT(osh, list_tail[ifidx],
						   pfirst);
					list_tail[ifidx] = pfirst;
				}
				num++;
...
deliver:
		...
		if (PKTLEN(osh, pkt) == 0) {
		    ...
		    if (dhd_prot_hdrpull
			(bus->dhd, &ifidx, pkt, reorder_info_buf,
			 &reorder_info_len) != 0) {
			DHD_ERROR(("%s: rx protocol error\n", __FUNCTION__));
			dhd_os_sdlock_rxq(bus->dhd);
			PKTFREE(bus->dhd->osh, pkt, FALSE);
		...
		if (bus->pktgen_minlen == bus->pktgen_maxlen) {
			...
			printf
			    ("%s: Tx Throughput %d kbps, Rx Throughput %d kbps\n",
			     __FUNCTION__,
			     (sent_pkts * bus->pktgen_len /
			      jiffies_to_msecs(time_lapse)) * 8,
			     (rcvd_pkts * bus->pktgen_len /
			      jiffies_to_msecs(time_lapse)) * 8);
		}
	}
...
static int dhdsdio_sdio_hang_war(struct dhd_bus *bus)
{
	...
	while ((bcmsdh_reg_read(bus->sdh, 0x18104808, 4) == 5) ||
		...
		DHD_ERROR(("%d: Read Value @ 0x18104808 = %x."
			   "\n", __LINE__, bcmsdh_reg_read(bus->sdh, 0x18104808,
							   4)));
		DHD_ERROR(("%d: Read Value @ 0x1810480C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810480C, 4)));
		DHD_ERROR(("%d: Read Value @ 0x18106808 = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x18106808, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810680C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810680C, 4)));
		DHD_ERROR(("%d: Read Value @ 0x18107808 = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x18107808, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810780C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810780C, 4)));
		DHD_ERROR(("%d: Read Value @ 0x18108808 = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x18108808, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810880C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810880C, 4)));
		DHD_ERROR(("%d: Read Value @ 0x18109808 = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x18109808, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810980C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810980C, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810C808 = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810c808, 4)));
		DHD_ERROR(("%d: Read Value @ 0x1810C80C = %x." "\n", __LINE__,
			   bcmsdh_reg_read(bus->sdh, 0x1810c80C, 4)));
	}
		     ...
		     uint16 devid)
{
	...
	if (((uint16) bus->sih->chip == BCM4324_CHIP_ID)
	    && (bus->sih->chiprev < 3))
...
dhdsdio_download_firmware(struct dhd_bus *bus, osl_t * osh, void *sdh)
{
	...
	if (bus->dhd->conf_path[0] == '\0')
		dhd_conf_set_conf_path_by_nv_path(bus->dhd, bus->dhd->conf_path,
						  bus->nv_path);
...
static int dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	while ((len = dhd_os_get_image_block((char *)memptr, MEMBLOCK, image))) {
		...
		if (dhd_msg_level & DHD_TRACE_VAL) {
			bcmerror =
			    dhdsdio_membytes(bus, FALSE, offset, memptr_tmp,
					     len);
			if (bcmerror) {
				DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n", __FUNCTION__, bcmerror, MEMBLOCK, offset));
				goto err;
...
err:
	...
	if (dhd_msg_level & DHD_TRACE_VAL) {
		if (memptr_tmp)
			MFREE(bus->dhd->osh, memptr_tmp,
			      MEMBLOCK + DHD_SDALIGN);
	}
...
#endif
			dhd_conf_set_fw_name_by_chip(dhdp, fw_path,
						     firmware_path);
