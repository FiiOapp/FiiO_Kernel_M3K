...
static uint
socram_banksize(si_info_t * sii, sbsocramregs_t * regs, uint8 idx,
		uint8 mem_type)
{
...
void si_chipcontrl_btshd0_4331(si_t * sih, bool on)
{
	si_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void si_chipcontrl_srom4360(si_t * sih, bool on)
{
	...
	if (on) {
		val &= ~(CCTRL4360_SECI_MODE |
			 ...
			 CCTRL4360_BT_LGCY_MODE | CCTRL4360_CORE2FEMCTRL4_ON);
...
void si_chipcontrl_epa4331_wowl(si_t * sih, bool enter_wowl)
{
	si_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void si_clk_pmu_htavail_set(si_t * sih, bool set_clear)
{
}

/* Re-enable synth_pwrsw resource in min_res_mask for 4313 */
void si_pmu_synth_pwrsw_4313_war(si_t * sih)
{
}

/* WL/BT control for 4313 btcombo boards >= P250 */
void si_btcombo_p250_4313_war(si_t * sih)
{
	si_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
void si_btcombo_43228_war(si_t * sih)
{
	si_info_t *sii;
	chipcregs_t *cc;
	uint origidx;
...
bool si_is_sprom_available(si_t * sih)
{
	...
	case BCM43428_CHIP_ID:
		return (sih->chipst & CST43228_OTP_PRESENT) !=
...
uint
si_core_wrapperreg(si_t * sih, uint32 coreidx, uint32 offset, uint32 mask,
		   uint32 val)
{
	uint origidx;
	uint ret_val;
