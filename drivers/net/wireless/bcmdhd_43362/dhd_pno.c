...
static inline bool is_dfs(uint16 channel)
{
...
static int _dhd_pno_clean(dhd_pub_t * dhd)
{
	int pfn = 0;
	int err;
...
static int _dhd_pno_suspend(dhd_pub_t * dhd)
{
	...
	DHD_PNO(("%s enter\n", __FUNCTION__));
	err =
	    dhd_iovar(dhd, 0, "pfn_suspend", (char *)&suspend, sizeof(suspend),
		      1);
	if (err < 0) {
...
static int
_dhd_pno_set(dhd_pub_t * dhd, const dhd_pno_params_t * pno_params,
	     dhd_pno_mode_t mode)
{
	int err = BCME_OK;
	...
	pfn_param.flags = ((PFN_LIST_ORDER << SORT_CRITERIA_BIT) |
			   (ENABLE << IMMEDIATE_SCAN_BIT) | (ENABLE <<
	...
	if (mode == DHD_PNO_LEGACY_MODE) {
		...
		if ((pno_params->params_legacy.pno_repeat != 0) ||
			...
			pfn_param.exp =
			    (uchar) (pno_params->params_legacy.
				     pno_freq_expo_max);
		}
		/* set up pno scan fr */
	...
	if (mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (mode == DHD_PNO_BATCH_MODE) {
			...
			if (pno_params->params_batch.scan_fr)
				pfn_param.scan_freq =
				    htod32(pno_params->params_batch.scan_fr);
		...
		if (combined_scan) {
			...
			if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
				...
				_params =
				    &(_pno_state->
				      pno_params_arr[INDEX_OF_HOTLIST_PARAMS]);
				if (_params->params_hotlist.scan_fr)
					pfn_param.scan_freq =
					    htod32(_params->params_hotlist.
						   scan_fr);
				pfn_param.bestn = 0;
				pfn_param.repeat = 0;
			}
	...
	if (mode == DHD_PNO_BATCH_MODE) {
		...
		err =
		    dhd_iovar(dhd, 0, "pfnmscan", (char *)&_tmp, sizeof(_tmp),
			      1);
		if (err < 0) {
	...
	err =
	    dhd_iovar(dhd, 0, "pfn_set", (char *)&pfn_param, sizeof(pfn_param),
		      1);
	if (err < 0) {
		DHD_ERROR(("%s : failed to execute pfn_set\n", __FUNCTION__));
		goto exit;
...
_dhd_pno_chan_merge(uint16 * d_chan_list, int *nchan,
		    uint16 * chan_list1, int nchan1, uint16 * chan_list2,
		    int nchan2)
{
	int err = BCME_OK;
	int i = 0, j = 0, k = 0;
		      ...
		      int *nchan, uint8 band, bool skip_dfs)
{
	...
	err =
	    dhd_wl_ioctl_cmd(dhd, WLC_GET_VALID_CHANNELS, chan_buf,
...
static int
_dhd_pno_convert_format(dhd_pub_t * dhd,
			struct dhd_pno_batch_params *params_batch, char *buf,
			int nbufsize)
{
	int err = BCME_OK;
	...
	list_for_each_entry_safe(siter, snext,
				 &params_batch->get_batch.
				 expired_scan_results_list, list) {
...
static int
_dhd_pno_clear_all_batch_results(dhd_pub_t * dhd, struct list_head *head,
				 bool only_last)
{
	int err = BCME_OK;
...
static int _dhd_pno_cfg(dhd_pub_t * dhd, uint16 * channel_list, int nchan)
{
	...
	err =
	    dhd_iovar(dhd, 0, "pfn_cfg", (char *)&pfncfg_param,
		      sizeof(pfncfg_param), 1);
	if (err < 0) {
...
static int
_dhd_pno_reinitialize_prof(dhd_pub_t * dhd, dhd_pno_params_t * params,
			   dhd_pno_mode_t mode)
{
	int err = BCME_OK;
	dhd_pno_status_info_t *_pno_state;
...
int dhd_pno_stop_for_ssid(dhd_pub_t * dhd)
{
	...
	if (_pno_state->pno_mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			...
			/* restart BATCH SCAN */
			err =
			    dhd_pno_set_for_batch(dhd, &_params->params_batch);
			if (err < 0) {
			...
			if (p_pfn_bssid == NULL) {
				DHD_ERROR(("%s : failed to allocate wl_pfn_bssid_t array" " (count: %d)", __FUNCTION__, _params->params_hotlist.nbssid));
				err = BCME_ERROR;
			...
			list_for_each_entry_safe(iter, next,
						 ...
						 bssid_list, list) {
				memcpy(&p_pfn_bssid->macaddr, &iter->macaddr,
				       ETHER_ADDR_LEN);
				p_pfn_bssid->flags = iter->flags;
				p_pfn_bssid++;
			}
			err =
			    dhd_pno_set_for_hotlist(dhd, p_pfn_bssid,
						    &_params->params_hotlist);
			if (err < 0) {
				_pno_state->pno_mode &= ~DHD_PNO_HOTLIST_MODE;
		     ...
		     uint16 * channel_list, int nchan)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_LEGACY_MODE)) {
		_pno_state->pno_mode |= DHD_PNO_LEGACY_MODE;
		err =
		    _dhd_pno_reinitialize_prof(dhd, _params,
					       DHD_PNO_LEGACY_MODE);
		if (err < 0) {
			...
			if (_params2->params_batch.nchan > 0 && nchan > 0) {
				err =
				    _dhd_pno_chan_merge(_chan_list, &tot_nchan,
							&_params2->params_batch.
							chan_list[0],
							_params2->params_batch.
							nchan, &channel_list[0],
							nchan);
				if (err < 0) {
					DHD_ERROR(("%s : failed to merge channel list" " between legacy and batch\n", __FUNCTION__));
					goto exit;
				}
			} else {
			...
			if (_params2->params_hotlist.nchan > 0 && nchan > 0) {
				err =
				    _dhd_pno_chan_merge(_chan_list, &tot_nchan,
							...
							chan_list[0],
							_params2->
							params_hotlist.nchan,
							&channel_list[0],
							nchan);
				if (err < 0) {
					DHD_ERROR(("%s : failed to merge channel list" " between legacy and hotlist\n", __FUNCTION__));
					goto exit;
				}
			}
		}
	...
	for (i = 0; i < nssid; i++) {
		...
		list_add_tail(&_pno_ssid->list,
			      &_params->params_legacy.ssid_list);
...
int
dhd_pno_set_for_batch(dhd_pub_t * dhd,
		      struct dhd_pno_batch_params *batch_params)
{
	...
	if (batch_params->band == WLC_BAND_2G
		...
		err = _dhd_pno_get_channels(dhd,
					    ...
					    chan_list[batch_params->nchan],
					    &rem_nchan, batch_params->band,
					    FALSE);
		if (err < 0) {
	...
	if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
		...
		p_ssid_list = kzalloc(sizeof(wlc_ssid_t) *
				      _params2->params_legacy.nssid,
				      GFP_KERNEL);
		if (p_ssid_list == NULL) {
			DHD_ERROR(("%s : failed to allocate wlc_ssid_t array (count: %d)", __FUNCTION__, _params2->params_legacy.nssid));
			err = BCME_ERROR;
			_pno_state->pno_mode &= ~DHD_PNO_LEGACY_MODE;
			goto exit;
		}
		...
		list_for_each_entry_safe(iter, next,
					 &_params2->params_legacy.ssid_list,
					 list) {
			p_ssid_list[i].SSID_len = iter->SSID_len;
			memcpy(p_ssid_list->SSID, iter->SSID,
			       p_ssid_list[i].SSID_len);
			i++;
		}
...
_dhd_pno_get_for_batch(dhd_pub_t * dhd, char *buf, int bufsize, int reason)
{
	...
	while (plbestnet->status != PFN_COMPLETE) {
		memset(plbestnet, 0, PNO_BESTNET_LEN);
		err =
		    dhd_iovar(dhd, 0, "pfnlbest", (char *)plbestnet,
			      PNO_BESTNET_LEN, 0);
		if (err < 0) {
			if (err == BCME_EPERM) {
		...
		for (i = 0; i < plbestnet->count; i++) {
			...
			if (timestamp
			    && (TIME_DIFF(timestamp, plnetinfo->timestamp)
				> TIME_MIN_DIFF))
...
convert_format:
		if (!list_empty
		    (&_params->params_batch.get_batch.
		     expired_scan_results_list)) {
			err =
			    _dhd_pno_convert_format(dhd, &_params->params_batch,
						    buf, bufsize);
			if (err < 0) {
		...
		if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
			...
			_params_legacy =
			    &(_pno_state->
			      ...
			      params_legacy);
			p_ssid_list =
			    kzalloc(sizeof(wlc_ssid_t) * _params_legacy->nssid,
				    GFP_KERNEL);
			if (p_ssid_list == NULL) {
				DHD_ERROR(("%s : failed to allocate wlc_ssid_t array (count: %d)", __FUNCTION__, _params_legacy->nssid));
				err = BCME_ERROR;
				_pno_state->pno_mode &= ~DHD_PNO_LEGACY_MODE;
				goto exit;
			}
			...
			/* convert dhd_pno_ssid to dhd_pno_ssid */
			list_for_each_entry_safe(iter, next,
						 &_params_legacy->ssid_list,
						 list) {
				p_ssid_list[i].SSID_len = iter->SSID_len;
				memcpy(p_ssid_list[i].SSID, iter->SSID,
				       p_ssid_list[i].SSID_len);
				i++;
			}
			err =
			    dhd_pno_set_for_ssid(dhd, p_ssid_list,
						 _params_legacy->nssid,
						 _params_legacy->scan_fr,
