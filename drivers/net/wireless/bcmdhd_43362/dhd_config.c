...
int dhd_conf_get_mac(dhd_pub_t * dhd, bcmsdh_info_t * sdh, uint8 * mac)
{
	...
	do {
		...
		if (config_msg_level & CONFIG_TRACE_LEVEL) {
			printf
			    ("%s: tpl_code=0x%02x, tpl_link=0x%02x, tag=0x%02x\n",
		...
		for (j = 0; j < mac_num; j++) {
			if (oui == mac_range[j].oui) {
				if (nic >= mac_range[j].nic_start
					...
					printf
					    ("%s: matched oui=0x%06X, nic=0x%06X\n",
...
void dhd_conf_set_fw_name_by_chip(dhd_pub_t * dhd, char *dst, char *src)
{
	...
	if (src[0] == '\0') {
		...
		bcm_strncpy_s(src, sizeof(fw_path), CONFIG_BCMDHD_FW_PATH,
			      MOD_PARAM_PATHLEN - 1);
		if (src[0] == '\0')
#endif
		{
...
void
dhd_conf_set_conf_path_by_nv_path(dhd_pub_t * dhd, char *conf_path,
				  char *nv_path)
{
	int i;
	...
	if (nv_path[0] == '\0') {
		...
		bcm_strncpy_s(conf_path, MOD_PARAM_PATHLEN - 1,
			      CONFIG_BCMDHD_NVRAM_PATH, MOD_PARAM_PATHLEN - 1);
		if (nv_path[0] == '\0')
#endif
		{
...
int dhd_conf_get_country(dhd_pub_t * dhd, wl_country_t * cspec)
{
	...
	if ((bcmerror =
	     dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, cspec, sizeof(wl_country_t),
			      FALSE, 0)) < 0)
...
int dhd_conf_set_roam(dhd_pub_t * dhd)
{
	...
	bcm_mkiovar("roam_off", (char *)&dhd->conf->roam_off, 4, iovbuf,
		    sizeof(iovbuf));
	...
	if (!dhd->conf->roam_off || !dhd->conf->roam_off_suspend) {
		...
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER,
				      dhd->conf->roam_trigger,
				      sizeof(dhd->conf->roam_trigger), TRUE,
				      0)) < 0)
		...
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_SCAN_PERIOD,
				      dhd->conf->roam_scan_period,
				      sizeof(dhd->conf->roam_scan_period), TRUE,
				      0)) < 0)
			CONFIG_ERROR(("%s: roam scan period setting failed %d\n", __FUNCTION__, bcmerror));

		printf("%s: Set roam_delta %d\n", __FUNCTION__,
		       dhd->conf->roam_delta[0]);
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_DELTA,
				      dhd->conf->roam_delta,
				      sizeof(dhd->conf->roam_delta), TRUE,
				      0)) < 0)
		...
		bcm_mkiovar("fullroamperiod",
			    (char *)&dhd->conf->fullroamperiod, 4, iovbuf,
			    sizeof(iovbuf));
		if ((bcmerror =
...
void dhd_conf_force_wme(dhd_pub_t * dhd)
{
	...
	if (dhd_bus_chip_id(dhd) == BCM43362_CHIP_ID && dhd->conf->force_wme_ac) {
		bcm_mkiovar("force_wme_ac", (char *)&dhd->conf->force_wme_ac, 4,
			    iovbuf, sizeof(iovbuf));
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0)) < 0)
...
void dhd_conf_get_wme(dhd_pub_t * dhd, edcf_acparam_t * acp)
{
	...
	CONFIG_TRACE(("%s: BK: aci %d aifsn %d ecwmin %d ecwmax %d size %d\n",
		      __FUNCTION__, acparam->ACI,
		      acparam->ACI & EDCF_AIFSN_MASK,
		      acparam->ECW & EDCF_ECWMIN_MASK,
		      (acparam->ECW & EDCF_ECWMAX_MASK) >> EDCF_ECWMAX_SHIFT,
		      sizeof(acp)));
	acparam = &acp[AC_BE];
...
void dhd_conf_update_wme(dhd_pub_t * dhd, edcf_acparam_t * acparam_cur, int aci)
{
	int bcmerror = -1;
	...
	acp->ECW =
	    ((ecwmax << EDCF_ECWMAX_SHIFT) & EDCF_ECWMAX_MASK) | (acp->
								  ECW &
								  EDCF_ECWMIN_MASK);
	acp->ECW =
	    ((acp->ECW & EDCF_ECWMAX_MASK) | (ecwmin & EDCF_ECWMIN_MASK));

	CONFIG_TRACE(("%s: mod aci %d aifsn %d ecwmin %d ecwmax %d size %d\n",
		      __FUNCTION__, acp->ACI, acp->ACI & EDCF_AIFSN_MASK,
		      acp->ECW & EDCF_ECWMIN_MASK,
		      (acp->ECW & EDCF_ECWMAX_MASK) >> EDCF_ECWMAX_SHIFT,
	...
	bcm_mkiovar("wme_ac_sta", (char *)acp, sizeof(edcf_acparam_t), iovbuf,
		    sizeof(iovbuf));
	if ((bcmerror =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), FALSE,
			      0)) < 0) {
...
void dhd_conf_set_stbc(dhd_pub_t * dhd)
{
	...
	if (dhd_bus_chip_id(dhd) == BCM4324_CHIP_ID && dhd->conf->stbc >= 0) {
		...
		bcm_mkiovar("stbc_tx", (char *)&stbc, 4, iovbuf,
			    sizeof(iovbuf));
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0)) < 0)
		...
		bcm_mkiovar("stbc_rx", (char *)&stbc, 4, iovbuf,
			    sizeof(iovbuf));
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0)) < 0)
...
void dhd_conf_set_phyoclscdenable(dhd_pub_t * dhd)
{
	...
	if (dhd_bus_chip_id(dhd) == BCM4324_CHIP_ID
	    && dhd->conf->phy_oclscdenable >= 0) {
		phy_oclscdenable = (uint) dhd->conf->phy_oclscdenable;
		printf("%s: set stbc_tx %d\n", __FUNCTION__, phy_oclscdenable);
		bcm_mkiovar("phy_oclscdenable", (char *)&phy_oclscdenable, 4,
			    iovbuf, sizeof(iovbuf));
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),
				      TRUE, 0)) < 0)
...
void dhd_conf_discard_pkt_filter(dhd_pub_t * dhd)
{
	...
	dhd->pktfilter[dhd->pktfilter_count] =
	    "111 0 0 12 0xFFFF00000000000000000000000000000000F0 0x080000000000000000000000000000000000E0";
	dhd->pktfilter_count++;
	/* discard IPv6 multicast address FF00::/8 */
	dhd->pktfilter[dhd->pktfilter_count] =
	    "112 0 0 12 0xFFFF000000000000000000000000000000000000000000000000FF 0x86DD000000000000000000000000000000000000000000000000FF";
	dhd->pktfilter_count++;
	/* discard Netbios pkt */
	dhd->pktfilter[dhd->pktfilter_count] =
	    "120 0 0 12 0xFFFF000000000000000000FF000000000000000000000000FFFF 0x0800000000000000000000110000000000000000000000000089";
	dhd->pktfilter_count++;
...
void dhd_conf_set_srl(dhd_pub_t * dhd)
{
	...
	if (dhd->conf->srl >= 0) {
		...
		printf("%s: set srl %d\n", __FUNCTION__, srl);
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_SRL, &srl, sizeof(srl), true,
				      0)) < 0)
...
void dhd_conf_set_lrl(dhd_pub_t * dhd)
{
	...
	if (dhd->conf->lrl >= 0) {
		...
		printf("%s: set lrl %d\n", __FUNCTION__, lrl);
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_LRL, &lrl, sizeof(lrl), true,
				      0)) < 0)
...
void dhd_conf_set_glom(dhd_pub_t * dhd)
{
	...
	if (dhd->conf->bus_txglom) {
		...
		bcm_mkiovar("bus:txglom", (char *)&bus_txglom, 4, iovbuf,
			    sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE,
				 0);
		if ((bcmerror =
...
void dhd_conf_set_spect(dhd_pub_t * dhd)
{
	...
	if (dhd->conf->spect >= 0) {
		...
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_DOWN, NULL, 0, TRUE, 0)) < 0)
			CONFIG_ERROR(("%s: WLC_DOWN setting failed %d\n",
				      __FUNCTION__, bcmerror));
		printf("%s: set spect %d\n", __FUNCTION__, spect);
		if ((bcmerror =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_SPECT_MANAGMENT, &spect,
				      sizeof(spect), true, 0)) < 0)
...
process_config_vars(char *varbuf, unsigned int len, char *pickbuf, char *param)
{
	...
	for (n = 0; n < len; n++) {
		...
		if (pick) {
			if (varbuf[n] == 0x9)
				continue;
			if (pick_column > 0 && pickbuf[pick_column - 1] == ' '
			    && varbuf[n] == ' ')
				continue;
...
void dhd_conf_read_log_level(dhd_pub_t * dhd, char *bufp, uint len)
{
	uint len_val;
	char pick[MAXSZ_BUF];
...
void dhd_conf_read_wme_ac_params(dhd_pub_t * dhd, char *bufp, uint len)
{
	uint len_val;
	char pick[MAXSZ_BUF];
	...
	if (conf->force_wme_ac) {
		...
		if (len_val) {
			conf->wme.aifsn[AC_BK] =
			    (int)simple_strtol(pick, NULL, 10);
...
void dhd_conf_read_roam_params(dhd_pub_t * dhd, char *bufp, uint len)
{
	uint len_val;
	char pick[MAXSZ_BUF];
	struct dhd_conf *conf = dhd->conf;
	...
	if (!conf->roam_off || !conf->roam_off_suspend) {
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "roam_scan_period=");
		if (len_val)
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "fullroamperiod=");
		if (len_val)
			conf->fullroamperiod =
			    (int)simple_strtol(pick, NULL, 10);
...
int dhd_conf_read_config(dhd_pub_t * dhd, char *conf_path)
{
	...
	if (len > 0 && len < MAXSZ_CONFIG) {
		...
		if (len_val) {
			...
			dhd->conf->fw_by_mac.count =
			    (uint32) simple_strtol(pch, NULL, 0);
			if (!
			    (mac_list =
			     kmalloc(sizeof(wl_mac_list_t) *
				     dhd->conf->fw_by_mac.count, GFP_KERNEL))) {
			...
			for (i = 0; i < dhd->conf->fw_by_mac.count; i++) {
				...
				printf("%s: name=%s, mac_count=%d\n",
				       __FUNCTION__, mac_list[i].name,
				       mac_list[i].count);
				if (!
				    (mac_range =
				     kmalloc(sizeof(wl_mac_range_t) *
					     mac_list[i].count, GFP_KERNEL))) {
					mac_list[i].count = 0;
					CONFIG_ERROR(("%s: kmalloc failed\n",
						      __FUNCTION__));
					break;
				...
				for (j = 0; j < mac_list[i].count; j++) {
					...
					mac_range[j].nic_end =
					    (uint32) simple_strtol(pch, NULL,
								   0);
					printf
					    ("%s: oui=0x%06X, nic_start=0x%06X, nic_end=0x%06X\n",
		...
		if (len_val) {
			pick_tmp = pick;
			pch = bcmstrtok(&pick_tmp, " ", 0);
			dhd->conf->nv_by_mac.count =
			    (uint32) simple_strtol(pch, NULL, 0);
			if (!
			    (mac_list =
			     kmalloc(sizeof(wl_mac_list_t) *
				     dhd->conf->nv_by_mac.count, GFP_KERNEL))) {
		...
		if (len_val) {
			dhd->conf->mimo_bw_cap =
			    (uint) simple_strtol(pick, NULL, 10);
		...
		if (len_val) {
			...
			memset(pick, 0, MAXSZ_BUF);
			len_val =
			    process_config_vars(bufp, len, pick, "regrev=");
			if (len_val)
				dhd->conf->cspec.rev =
				    (int32) simple_strtol(pick, NULL, 10);
		}
		...
		if (len_val) {
			...
			while (pch != NULL && i < WL_NUMCHANNELS) {
				dhd->conf->channels.channel[i] =
				    (uint32) simple_strtol(pch, NULL, 10);
				pch = bcmstrtok(&pick_tmp, " ,.-", 0);
				i++;
			}
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "keep_alive_period=");
		if (len_val) {
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "phy_oclscdenable=");
		if (len_val) {
			dhd->conf->phy_oclscdenable =
			    (int)simple_strtol(pick, NULL, 10);
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "dhd_master_mode=");
		if (len_val) {
			if (!strncmp(pick, "0", len_val))
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "pkt_filter_add=");
		pick_tmp = pick;
		if (len_val) {
			pch = bcmstrtok(&pick_tmp, ",.-", 0);
			i = 0;
			while (pch != NULL && i < DHD_CONF_FILTER_MAX) {
				strcpy(&dhd->conf->pkt_filter_add.filter[i][0],
				       pch);
				printf("%s: pkt_filter_add[%d][] = %s\n",
				       __FUNCTION__, i,
				       &dhd->conf->pkt_filter_add.filter[i][0]);
				pch = bcmstrtok(&pick_tmp, ",.-", 0);
				i++;
			}
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "pkt_filter_del=");
		pick_tmp = pick;
		if (len_val) {
			pch = bcmstrtok(&pick_tmp, " ,.-", 0);
			i = 0;
		...
		memset(pick, 0, MAXSZ_BUF);
		len_val =
		    process_config_vars(bufp, len, pick, "ampdu_ba_wsize=");
		if (len_val) {
			dhd->conf->ampdu_ba_wsize =
			    (int)simple_strtol(pick, NULL, 10);
		...
		if (len_val) {
			if (!strncmp(pick, "1", len_val))
				dhd->conf->deepsleep = TRUE;
			else
				dhd->conf->deepsleep = FALSE;
...
void dhd_conf_wifi_stop(struct net_device *dev)
{
	if (!dev) {
		CONFIG_ERROR(("%s: dev is null\n", __FUNCTION__));
		return;
...
int dhd_conf_wifi_power(bool on)
{
	int ret = 0;
...
void dhd_conf_power_workqueue(struct work_struct *work)
{
	int ret;
	struct sdio_early_suspend_info *sdioinfo =
	    container_of(work, struct sdio_early_suspend_info, tqueue.work);
...
void dhd_conf_early_suspend(struct early_suspend *h)
{
	struct sdio_early_suspend_info *sdioinfo =
	    container_of(h, struct sdio_early_suspend_info, sdio_early_suspend);
...
void dhd_conf_late_resume(struct early_suspend *h)
{
	struct sdio_early_suspend_info *sdioinfo =
	    container_of(h, struct sdio_early_suspend_info, sdio_early_suspend);
...
void dhd_conf_register_wifi_suspend(struct sdio_func *func)
{
	...
	if (func->num == 2) {
		...
		sdioinfo[func->num].sdio_early_suspend.level =
		    EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 30;
		sdioinfo[func->num].sdio_early_suspend.suspend =
		    dhd_conf_early_suspend;
		sdioinfo[func->num].sdio_early_suspend.resume =
		    dhd_conf_late_resume;
		register_early_suspend(&sdioinfo[func->num].sdio_early_suspend);
		INIT_DELAYED_WORK(&sdioinfo[func->num].tqueue,
				  dhd_conf_power_workqueue);
	}
#endif
}
...
void dhd_conf_unregister_wifi_suspend(struct sdio_func *func)
{
	...
	if (func->num == 2) {
		if (sdioinfo[func->num].sdio_early_suspend.suspend) {
			unregister_early_suspend(&sdioinfo[func->num].
						 sdio_early_suspend);
