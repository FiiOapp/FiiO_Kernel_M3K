	...
	if (entry != NULL) {
		if (pkt_type == eWLFC_PKTTYPE_SUPPRESSED) {
			...
			if (rc != BCME_OK) {
				...
				dhd_wlfc_hanger_poppkt(ctx->hanger, hslot,
						       &pktout, 1);
			   ...
			   int header_needed, uint32 * slot)
{
	...
	*slot = hslot;
	...
	if (header_needed) {
		...
		if (rc == BCME_OK) {
			hslot =
			    WLFC_PKTID_HSLOT_GET(DHD_PKTTAG_H2DTAG(PKTTAG(p)));
			dhd_wlfc_hanger_get_genbit(ctx->hanger, p, hslot, &gen);

			WLFC_PKTFLAG_SET_GENERATION(htod, gen);
			free_ctr =
			    WLFC_PKTID_FREERUNCTR_GET(DHD_PKTTAG_H2DTAG
						      (PKTTAG(p)));
			/* push new header */
...
void *_dhd_wlfc_pktq_peek_tail(struct pktq *pq, int *prec_out)
{
	int prec;
...
bool
_dhd_wlfc_prec_enq_with_drop(dhd_pub_t * dhdp, struct pktq * pq, void *pkt,
			     int prec)
{
	void *p = NULL;
...
_dhd_wlfc_enque_delayq(athost_wl_status_info_t * ctx, void *pktbuf, int prec)
{
	...
	if (pktbuf != NULL) {
		...
		if (_dhd_wlfc_prec_enq_with_drop
		    (ctx->dhdp, &entry->psq, pktbuf, (prec << 1))
		    == FALSE) {
			WLFC_DBGMESG(("D"));
...
static int
dhd_wlfc_compressed_txstatus_update(dhd_pub_t * dhd, uint8 * pkt_info,
				    uint8 len)
{
	uint8 status_flag;
	uint32 status;
	...
	while (count < len) {
		...
		if (DHD_PKTTAG_CREDITCHECK(PKTTAG(pktbuf))) {
			...
			if (!entry) {
				...
				entry =
				    _dhd_wlfc_find_table_entry(wlfc, pktbuf);
			}
...
static int dhd_wlfc_dbg_senum_check(dhd_pub_t * dhd, uint8 * value)
{
	uint32 timestamp;
...
static void
dhd_wlfc_reorderinfo_indicate(uint8 * val, uint8 len, uchar * info_buf,
			      uint * info_len)
{
	if (info_len) {
		if (info_buf) {
	...
	if (remainder) {
		while ((processed < (WLFC_MAX_PENDING_DATALEN * 2))
			...
			if (type == WLFC_CTL_TYPE_COMP_TXSTATUS)
				dhd_wlfc_compressed_txstatus_update(dhd, value,
								    len);

			else if (type == WLFC_CTL_TYPE_HOST_REORDER_RXPKTS)
				dhd_wlfc_reorderinfo_indicate(value, len,
							      reorder_info_buf,
							      reorder_info_len);
...
void dhd_wlfc_cleanup(dhd_pub_t * dhd, ifpkt_cb_t fn, int arg)
{
	...
	for (i = 0; i < total_entries; i++) {
		if (table[i].occupied
		    && (fn == NULL || (arg == table[i].interface_id))) {
			if (table[i].psq.len) {
				WLFC_DBGMESG(("%s(): DELAYQ[%d].len = %d\n",
					      __FUNCTION__, i,
					      table[i].psq.len));
				/* release packets held in DELAYQ */
				pktq_flush(wlfc->osh, &table[i].psq, TRUE, fn,
					   arg);
			}
			if (fn == NULL)
				table[i].occupied = 0;
