	...
	while (eromptr < eromlim) {
		...
		if (asd == 0) {
			do {
				...
				asd =
				    get_asd(sih, &eromptr, 0, 0, AD_ST_BRIDGE,
					    &addrl, &addrh, &sizel, &sizeh);
				if (asd != 0)
					br = TRUE;
				else {
					if (br == TRUE) {
						break;
						   ...
						   || (sizel != SI_CORE_SIZE)) {
						SI_ERROR(("addrh = 0x%x\t sizeh = 0x%x\t size1 =" "0x%x\n", addrh, sizeh, sizel));
...
void ai_coreaddrspaceX(si_t * sih, uint asidx, uint32 * addr, uint32 * size)
{
	...
	asd =
	    get_asd(sih, &eromptr, 0, 0, AD_ST_SLAVE, &addrl, &addrh, &sizel,
		    &sizeh);
	if (asd == 0) {
	...
	for (i = 1; i < nsp; i++) {
		...
		do {
			asd =
			    get_asd(sih, &eromptr, i, j, AD_ST_SLAVE, &addrl,
				    &addrh, &sizel, &sizeh);
			if (asd == 0)
				break;
...
void ai_core_disable(si_t * sih, uint32 bits)
{
	...
	if (status != 0) {
		...
		SPINWAIT(((status =
			   R_REG(sii->osh, &ai->resetstatus)) != 0), 10000);
...
uint32 ai_core_sflags(si_t * sih, uint32 mask, uint32 val)
{
	...
	sii = SI_INFO(sih);
	if (BCM47162_DMP()) {
		SI_ERROR(("%s: Accessing MIPS DMP register (iostatus) on 47162a0", __FUNCTION__));
		return 0;
	}
	if (BCM5357_DMP()) {
