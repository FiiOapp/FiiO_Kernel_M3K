	       ...
	       union iwreq_data *cwrq, char *extra)
{
	...
	if ((err =
		...
		goto done;
	if ((err =
	     dev_wlc_ioctl(dev, WLC_GET_BANDLIST, band, sizeof(band))) < 0)
		goto done;
			     ...
			     struct iw_request_info *info, wl_bss_info_t * bi)
{
	...
	if (bi->ie_length) {
		...
		while ((ie = bcm_parse_tlvs(ptr, ptr_len, DOT11_MNG_WAPI_ID))) {
			...
			wpa_snprintf_hex(buf + 12, 2 * ie->len + 1, ie->data,
					 ie->len);
			event =
			    IWE_STREAM_ADD_POINT(info, event, end, &iwe, buf);
			kfree(buf);
	       ...
	       struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++) {
		...
		WL_SCAN(("%s: BSSID=" MACSTR
			 ", channel=%d, RSSI=%d, merge broadcast SSID=\"%s\"\n",
			 __FUNCTION__, MAC2STR(bi->BSSID.octet), channel, rssi,
			 bi->SSID));
		     ...
		     struct iw_point *dwrq, char *extra)
{
	...
	while (p_buf != iscan->list_cur) {
		...
		for (ii = 0; ii < list->count && apcnt < IW_MAX_AP;
			...
			WL_SCAN(("%s: BSSID=" MACSTR
				 ", channel=%d, RSSI=%d, merge broadcast SSID=\"%s\"\n",
				 __FUNCTION__, MAC2STR(bi->BSSID.octet),
				 channel, rssi, bi->SSID));
		  ...
		  struct iw_param *vwrq, char *extra)
{
	...
	case IW_AUTH_WAPI_ENABLED:
		...
		if (paramval) {
			...
			if ((error =
			     dev_wlc_intvar_set(dev, "wpa_auth",
	...
	case WLC_E_NDIS_LINK:
		...
		if (!(flags & WLC_EVENT_MSG_LINK)) {
			printk("%s: Link Down with BSSID=" MACSTR "\n",
			       __FUNCTION__, MAC2STR((u8 *) wrqu.addr.sa_data));
			bzero(wrqu.addr.sa_data, ETHER_ADDR_LEN);
			bzero(&extra, ETHER_ADDR_LEN);
		} else {
