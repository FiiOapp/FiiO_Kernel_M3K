...
int bcmsdh_cis_read(void *sdh, uint func, uint8 * cis, uint length)
{
	...
	if (ascii) {
		...
		for (tmp_ptr = tmp_buf, ptr = cis; ptr < (cis + length - 4);
		     tmp_ptr++) {
			ptr +=
			    snprintf((char *)ptr, (cis + length - ptr - 4),
				     "%.2x ", *tmp_ptr & 0xff);
...
void bcmsdh_glom_post(void *sdh, uint8 * frame, void *pkt, uint len)
{
	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *) sdh;
	sdioh_glom_post(bcmsdh->sdioh, frame, pkt, len);
}

void bcmsdh_glom_clear(void *sdh)
{
	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *) sdh;
