...
#include <../../kernel/sound/soc/ingenic/asoc-v13/asoc-aic-v13.h>
...
static char driver_name[] = "FM_contorl";	
static unsigned int fm_tune=9680;
...
static int si4713_powerup(struct si4713_device *sdev)
{
	 ...
	 */
	const u8 args[2] = {
		POWER_UP_IN_FUNC_FMRX,
		POWER_UP_IN_OPMODE_RX_DIGITAL,
	};
...
static int fmSeekStart(struct si4713_device *sdev,u8 seekUp, u8 wrap)
{
    int err;
	u8 val[1];
	/*
	 ...
	 */
	 u8 args[1] = {
		FM_SEEK_START_IN_SEEKUP,
	};
	// if(seekUp) args[1] |= FM_SEEK_START_IN_SEEKUP;
	//if(wrap)   args[1] |= FM_SEEK_START_IN_WRAP;
	err = si4713_send_command(sdev, FM_SEEK_START1,
				  args, ARRAY_SIZE(args), val,
				  ARRAY_SIZE(val), DEFAULT_TIMEOUT);
	...
	return 0;//compose_u16(args[1], args[2]);
}
...
//-----------------------------------------------------------------------------
static u8 fmTuneStatus(struct si4713_device *sdev,u8 cancel, u8 intack)
{
	int err;
	u8 BLTF;
	 ...
	 */
	 u8 args[1]= {
		FM_TUNE_STATUS_IN_INTACK,
	};
err = si4713_send_command(sdev, FM_TUNE_STATUS,
				  args, ARRAY_SIZE(args), val,
				  ARRAY_SIZE(val), DEFAULT_TIMEOUT);
    ...
    return BLTF   = !!(val[1] & FM_TUNE_STATUS_OUT_BTLF);
    
}


static int si47xxFMRX_get_frequency(struct si4713_device *sdev)
{
	int err;
	u16 Freq;
...
err = si4713_send_command(sdev, FM_TUNE_STATUS,
    ...
    return Freq   = ((u16)val[2] << 8) | (u16)val[3];
...
static int si4713_tx_tune_freq(struct si4713_device *sdev, u16 frequency)
{
 ...
 int si4713_rx_tune_freq(struct si4713_device *sdev, u16 frequency)
{
	int err;
	u8 val[1];
	/*
	...
	err = si4713_send_command(sdev, FM_TUNE_FREQ,
				  args, ARRAY_SIZE(args), val,
				  ARRAY_SIZE(val), DEFAULT_TIMEOUT);
...
EXPORT_SYMBOL(si4713_rx_tune_freq);
/*
...
static void si4713_set_mute(struct si4713_device *sdev, u16 mute)
{
	if(mute)
    	si4713_write_property(sdev,RX_HARD_MUTE, 
                                RX_HARD_MUTE_RMUTE_MASK | RX_HARD_MUTE_LMUTE_MASK);
    else
    	si4713_write_property(sdev,RX_HARD_MUTE, 0);
}
...
static ssize_t test_fm_show(struct device *device, struct device_attribute *attr, char *buf)
{

	return 0;
}

static ssize_t test_fm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{

	return 0;
}
static ssize_t fm_show(struct device *device, struct device_attribute *attr, char *buf)
{
	return 0;
}
static ssize_t fm_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	
	return 0;
}			
static struct device_attribute fm_device_attrs[] = {
	__ATTR(reg, S_IRUGO | S_IWUSR, fm_show, fm_store),
		
	__ATTR(test_eq, S_IRUGO | S_IWUSR, test_fm_show, test_fm_store),
};

static int fm_device_attr_register(struct miscdevice  *fm_opt)
{
	int error = 0;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(fm_device_attrs); i++) {
		error = device_create_file(fm_opt->this_device, &fm_device_attrs[i]);
	...
	if (error) {
		while (--i >= 0)
			device_remove_file(fm_opt->this_device, &fm_device_attrs[i]);
	}

	return 0;
}

static int fm_device_attr_unregister(struct miscdevice  *fm_opt)
{
	unsigned int i;
...
static int fm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	 int err = 0;
    int ret = 0;
    int ioarg = 0;
    u16 Freq;
    printk("enter  fm_ioctl\n");
	  
	 /* 根据命令，执行相应的操作 */
    switch(cmd) {
		case FM_CH_SEEK_UP://freq ++
		//static unsigned int fm_tune=9680;
		Freq=si47xxFMRX_get_frequency(sdev);
		Freq+=1;
		 printk("begin seek up to %d\n",Freq);
		si4713_rx_tune_freq(sdev,Freq);
		break;
		case FM_CH_SEEK_DOWN: //freq--
		Freq=si47xxFMRX_get_frequency(sdev);
		Freq-=1;
		 printk("begin seek down to %d\n",Freq);
		 si4713_rx_tune_freq(sdev,Freq);
		 break;
		case FM_SEEK_START:
		 printk("begin seek start\n");
		 si4713_set_mute(sdev,1);
		fmSeekStart(sdev,1, 0);
		fmSeekStart(sdev,0, 1);
		Freq=si47xxFMRX_get_frequency(sdev);
		printk("seek_fail=%d,curren_freq=%d\n",fmTuneStatus(sdev,0,1),Freq);
		si4713_rx_tune_freq(sdev,Freq);
		si4713_set_mute(sdev,0);
		break;
		case FM_GET_FREQ:
		Freq=si47xxFMRX_get_frequency(sdev);
		ioarg = Freq;
...
static struct miscdevice fm_opt = {
					...
					const struct i2c_device_id *id)
{
	...
	int rval, i;
	 printk("======FM_EN request=======\n");
       if(FM_EN > 0){
        rval = gpio_request(FM_EN, "FM_EN");
        if(rval) {
	...
	gpio_direction_output(FM_EN,1);
	 ...
	 rval = misc_register(&fm_opt);
	if (rval < 0)
	{
		printk(KERN_ERR"register misc fm opt failed.\n");
		return rval;
	}
	fm_device_attr_register(&fm_opt);
	rtc32k_enable();
	...
	sdev->gpio_reset = -1;
	 if(FM_RST > 0){
        rval = gpio_request(FM_RST, "FM_RST");
        if (rval) {
            printk(KERN_ERR "can's request FM_RST\n");
           return rval;
        }
    
		sdev->gpio_reset =FM_RST;
	...
	printk("si4713_write_property1\n");
	si4713_write_property(sdev,DIGITAL_OUTPUT_SAMPLE_RATE,0xBB80);
	printk("si4713_write_property2\n");
    si4713_write_property(sdev,DIGITAL_OUTPUT_FORMAT, DIGITAL_MODE_I2S);
    printk("si4713_write_property3\n");
	si4713_write_property(sdev,FM_SEEK_TUNE_SNR_THRESHOLD, 1);//3
	printk("si4713_write_property4\n");
    si4713_write_property(sdev,FM_SEEK_TUNE_RSSI_THRESHOLD, 2);//20
    printk("si4713_write_property5\n");
     si4713_write_property(sdev,FM_RDS_INTERRUPT_SOURCE, 
					FM_RDS_INTERRUPT_SOURCE_SYNCFOUND_MASK); // RDS Interrupt

		// Enable the RDS and allow all blocks so we can compute the error
        // rate later.
        printk("si4713_write_property6\n");
         si4713_write_property(sdev,FM_RDS_CONFIG, FM_RDS_CONFIG_RDSEN_MASK |
			(3 << FM_RDS_CONFIG_BLETHA_SHFT) |
			(3 << FM_RDS_CONFIG_BLETHB_SHFT) |
			(3 << FM_RDS_CONFIG_BLETHC_SHFT) |
			(3 << FM_RDS_CONFIG_BLETHD_SHFT));
			printk("si4713_write_property7\n");
         si4713_write_property(sdev,FM_DEEMPHASIS, FM_DEEMPH_75US); // Deemphasis
         printk("si4713_write_property8\n");
         si4713_write_property(sdev,RX_HARD_MUTE, 0);

        // Set the volume to the passed value
        printk("si4713_write_property9\n");
        si4713_write_property(sdev,RX_VOLUME, 0x003F & RX_VOLUME_MASK);
        printk("si4713_write_property10\n");
        si4713_rx_tune_freq(sdev,8750);
	return 0;
...
static int si4713_remove(struct i2c_client *client)
{
	...
	struct si4713_device *sdev = to_si4713_device(sd);
     fm_device_attr_unregister(&fm_opt);
	if (sdev->power_state)
