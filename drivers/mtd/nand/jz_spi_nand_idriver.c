...
static int jz_spi_nandflash_get_status(struct jz_spi_nandflash *flash,unsigned int *status)
{
	...
	transfer[1].rx_buf = recv;
	transfer[1].len = sizeof(recv);
...
struct jz_spi_support *jz_spi_flash_probe(struct spi_device *spi, int id_count)
{
	int ret,i;
	unsigned char send_command[2], recv_command[5];
	struct spi_message message;
	struct spi_transfer transfer[2];
...
static int jz_spi_nandflash_probe(struct spi_device *spi)
{
    ...
    for(id_count=1; id_count<=2; id_count++)
    {
        spi_flash = jz_spi_flash_probe(spi, id_count);
        if( spi_flash )
        {
            break;
        }
