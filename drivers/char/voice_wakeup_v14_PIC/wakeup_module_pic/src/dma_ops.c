...
struct dma_desc *suspend_desc;  /* pointer to desc in tcsm */
...
void dma_config_normal(void)
{
	...
	cpm_start_dma_clock(0);

	suspend_desc = (struct dma_desc *)get_dma_desc_addr();
	src_buf = (int *)DMIC_RX_FIFO;
	//dst_buf = (int *)VOICE_TCSM_DATA_BUF;
	dst_buf = (int *)(get_rx_fifo()->xfer.buf);

	config.type = DMIC_REQ_TYPE; /* dmic receive request */
	...
	config.desc = V_TO_P(suspend_desc);
//	config.tie = 1;
	build_circ_descs(suspend_desc);
	pdma_config(&config);
#ifdef DMA_VOICE_DEBUG
	dump_descs(suspend_desc);
#endif
