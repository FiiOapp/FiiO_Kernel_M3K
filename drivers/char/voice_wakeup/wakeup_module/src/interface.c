...
void set_record_buffer(unsigned char *buffer, int len)
{
...
unsigned int get_record_buffer(void)
{
	if(g_record_buffer != NULL) {
		return (unsigned int)g_record_buffer;
	} else {
...
void set_desc_addr(unsigned char *desc, unsigned int len)
{
	g_desc_addr = desc;
	g_desc_size = len;
