...
static ssize_t jzmmc_fiiotfstate_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	ssize_t count = 0;
	struct jzmmc_host *host = dev_get_drvdata(dev);
	int state = get_pin_status(&host->pdata->gpio->cd);

	count = sprintf(buf, "%d",state);

	//printk("\n%s count=%d state=%d.\n",__func__,count,state);
...
static DEVICE_ATTR(fiiotfstate, S_IRUSR | S_IRGRP | S_IROTH, jzmmc_fiiotfstate_show, NULL);
	...
	case REMOVABLE:
		if (gpio_is_valid(card_gpio->cd.num)) {
			...
			if(!timer_pending(&host->detect_timer)){
				...
				mod_timer(&host->detect_timer, jiffies+HZ);//delay 1s
			}
